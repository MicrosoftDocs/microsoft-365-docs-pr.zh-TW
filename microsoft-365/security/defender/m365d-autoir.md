...
如果您的組織使用 [Microsoft 365 Defender](microsoft-365-defender.md)，當偵測到惡意或可疑的活動或專案時，您的安全性作業小組會在 microsoft 365 安全性中心內收到警示。 針對可能來自的威脅，在這種情況下，安全性小組通常會面臨解決大量提醒問題的難題。 幸運的是，Microsoft 365 Defender 包含自動調查和回應 (空氣) 功能，可協助您的安全性運作小組更有效率地處理威脅。
...
假設您在第1層或第2層安全性運作小組中有虛擬分析員。 虛擬分析員會模仿安全性作業要採取哪些理想步驟來調查和補救威脅。 虛擬分析員可以全天候運作，並可在大量進行調查和威脅修復時採取大量的功能。 這類虛擬分析員可大幅減少回應時間，以騰出安全性運作小組的其他重要威脅或戰略性專案。 如果此案例好像是科學 fiction，就不是！ 這類虛擬分析員是 Microsoft 365 Defender 套件的一部分，而且其名稱是 *自動調查和回應*。
...
如需詳細資訊，請參閱 [在 Microsoft 365 Defender 中查看修正動作](m365d-remediation-actions.md)。
...
在 Microsoft 365 Defender 中，每個自動調查都會將每個 Microsoft Defender 的信號與 Microsoft Defender 的身分識別、Microsoft Defender for Endpoint 和 Microsoft Defender for Office 365 相關聯，如下表摘要： 
