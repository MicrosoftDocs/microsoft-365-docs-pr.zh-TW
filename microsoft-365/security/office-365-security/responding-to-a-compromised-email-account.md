...
使用的認證會控制對 Microsoft 365 信箱、資料和其他服務的存取，例如使用者名稱和密碼或 PIN。 當預期使用者以外的其他人竊取這些認證時，遭竊的認證會被視為遭入侵。 攻擊者可以使用這些認證以原始使用者身分登入，並執行非法動作。
...
如果使用者報告上述的任何症狀，則應該執行進一步調查。 [Microsoft 365 Defender](https://security.microsoft.com) 和 Azure 入口網站會提供工具，以協助您調查懷疑可能遭入侵之使用者帳戶的活動。

- **Microsoft 365 Defender 中的整合稽核記錄**：檢閱可疑帳戶的所有活動，方法是篩選日期範圍自立即發生可疑活動之前到目前日期的結果。 請勿在搜尋期間篩選活動。

- **EAC 中的系統管理員稽核記錄**：在 Exchange Online 中，您可以使用 Exchange 系統管理中心 (EAC) 來搜尋及檢閱系統管理員稽核記錄中的項目。 系統管理員稽核記錄會根據 Exchange Online PowerShell Cmdlet 記錄由系統管理員，以及已被指派系統管理權限的使用者所執行的特定動作。 系統管理員稽核記錄中的項目會提供執行的 Cmdlet、使用的參數、Cmdlet 的執行者，以及受影響的物件的相關資訊。
...
>
> - 請勿重新使用任何最近使用的五個密碼。即使密碼歷程記錄需求可讓您重複使用最近的密碼，您應該選取攻擊者無法猜測的項目。
>
...
1. 在 <https://admin.microsoft.com> 開啟 Microsoft 365 系統管理中心，然後移至 [使用者 **]** \> [作用中使用者 **]**。

2. 尋找並選取使用者帳戶，按一下 [更多圖示![]](../../media/ITPro-EAC-MoreOptionsIcon.png)，然後選取 [編輯登入狀態 **]**。

3. 在顯示的 **[封鎖登入]** 窗格中，選取 **[封鎖此使用者，使其無法登入]**，然後按一下 **[儲存變更]**。

4. 在 <https://admin.exchange.microsoft.com> 開啟 Exchange 系統管理中心 (EAC)，然後移至 [收件者 **]** \> [信箱 **]**。

5. 尋找並選取使用者。 在隨之開啟的信箱詳細資料飛出視窗中，執行下列步驟：
   - 在 [電子郵件應用程式 **]** 區段中，將切換向右移至 [停用![]](../../media/scc-toggle-on.png) 以封鎖所有可用的設定：
     - **Outlook 網頁版**
     - **Outlook 電腦版 (MAPI)**
     - **Exchange Web 服務**
...
1. 使用全域系統管理員帳戶在 <https://admin.microsoft.com> 開啟 Microsoft 365 系統管理中心，然後執行以下步驟：
   1. 移至 [使用者 **]** \> [作用中使用者 **]**。
   2. 尋找並選取使用者帳戶，按一下 [更多圖示![]](../../media/ITPro-EAC-MoreOptionsIcon.png)，然後選取 [管理角色 **]**。
   3. 移除指派給帳戶的任何系統管理角色。 完成後，按一下 [儲存變更 **]**。

2. 在 <https://security.microsoft.com> 開啟 Microsoft 365 Defender 入口網站 ，然後執行下列步驟：
   1. 移至 [權限與角色 **]** \> [電子郵件與共同作業角色 **]** \> [角色 **]**。
   2. 在 [權限 **]** 頁面上，選取清單中的每個角色群組，並在顯示之詳細資料飛出視窗的 [成員 **]** 區段中尋找該使用者帳戶。 如果角色群組包含該使用者帳戶，請執行下列步驟：
      1. 按一下 [成員 **]** 區段中的 [編輯 **]**。
      2. 在顯示之 [編輯選擇成員 **]** 飛出視窗上，按一下 [編輯 **]**。
      3. 在顯示之 [選擇成員 **]** 飛出視窗上，按一下 [移除 **]**。
      4. 在顯示的飛出視窗中選取該使用者帳戶，然後按一下 [移除 **]**。

         完成時，依序按一下 [完成 **]** 和 [儲存 **]**，然後按一下 [關閉 **]**。

3. 在 <https://admin.exchange.microsoft.com> 開啟 EAC，然後執行下列步驟：
   1. 選取 [角色 **]** \> [系統管理員角色 **]**。
   2. 在 [系統管理員角色 **]** 頁面上手動選取每個角色群組，然後選取詳細資料窗格中的 [已指派 **]** 索引標籤，以驗證使用者帳戶。 如果角色群組包含該使用者帳戶，請執行下列步驟：
      1. 選取使用者帳戶。
      2. 按一下 ![刪除圖示](../../media/m365-cc-sc-delete-icon.png).
