...
ATP 反網路釣魚檢查內送郵件的指示，郵件可能是網路釣魚。 每當 ATP 原則所涵蓋的使用者 (安全附件、安全連結或反網路釣魚) 內送郵件會由多部機器教學模型評估，以進行分析，以判斷原則是否套用至郵件，並根據設定的原則採取適當的動作。
...
ATP 反網路釣魚功能是[高級威脅防護](office-365-atp.md)的一部分;不過，當反網路釣魚原則定義時，ATP 反網路釣魚防護會套用。  (其中一個範例是模擬型原則。 ) 請參閱[設定 Office 365 中的反網路釣魚原則](set-up-anti-phishing-policies.md)。
...
|Lee 是 Contoso 銷售部門的員工。 「他的組織」有適當的 ATP 反網路釣魚原則，只適用于財務員工。|否。 在此情況下，會將 ATP 反網路釣魚 (機器模型和模擬保護) 套用到財務員工，但其他員工（包括銷售部門）將不會。|
