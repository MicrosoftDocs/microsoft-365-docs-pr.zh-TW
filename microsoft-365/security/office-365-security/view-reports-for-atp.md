...
該報告提供包含惡意內容的獨特電子郵件的匯總計數，例如檔案或網站位址（URLs），該惡意內容已封鎖反惡意程式碼引擎、[零小時的自動清除（ZAP）](zero-hour-auto-purge.md)和 atp 功能（如[atp 安全連結](atp-safe-links.md)、 [atp 安全附件](atp-safe-attachments.md)及[atp 反網路釣魚功能](atp-anti-phishing.md)）。 

資訊的篩選和細分可讓您在此報告中進一步細化資訊的類別。 具體而言，*電子郵件 > 網路釣魚*和*電子郵件 > 惡意程式碼視圖*中，都有「細分者」功能表。 它會將資料分解成：
