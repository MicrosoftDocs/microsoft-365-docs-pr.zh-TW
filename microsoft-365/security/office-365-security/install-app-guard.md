...
| 防止使用者移除檔上的應用程式防護保護               | 啟用此原則將會移除 Office 應用程式經驗) 中的選項 (，以停用應用程式防護保護或開啟應用程式防護之外的檔案。 <br><br>**附注：** 使用者仍可略過此原則，方法是手動移除檔案中的 web 標記，或是將檔移至信任位置。 |
...
* .NET 更新導致檔無法在應用程式防護中開啟。 作為變通方法，當您遇到此問題時，使用者可以重新開機其裝置。
    深入瞭解 [當嘗試開啟 Windows Defender 應用程式防護或 Windows 沙箱時，收到錯誤訊息時](https://support.microsoft.com/help/4575917/receiving-an-error-message-when-attempting-to-open-windows-defender-ap)，有關問題的詳細資訊。
