---
title: What's new in Office 365 進階威脅防護
description: 列出的新功能和 Office 365 ATP 中的功能
keywords: what's new in Office 365 atp，ga，通常可用功能，可以使用新
search.appverid: met150
ms.service: O365-seccomp
ms.sitesec: library
ms.pagetype: security
f1.keywords:
- NOCSH
ms.author: deniseb
author: denisebmsft
localization_priority: Normal
manager: dansimp
audience: ITPro
ms.collection: M365-security-compliance
ms.topic: conceptual
ms.date: 02/21/2020
ms.openlocfilehash: 09cf8e74cb52d75d454fec371ee965978851b5e5
ms.sourcegitcommit: 74bf600424d0cb7b9d16b4f391aeda7875058be1
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 02/22/2020
ms.locfileid: "42230066"
---
# <a name="whats-new-in-office-365-atp"></a>Office 365 ATP 中的新功能

適用於：****

- [Office 365 進階的威脅防護 (Office 365 ATP)](office-365-atp.md)

本文列出最新版的 Office 365 ATP 中的新功能。 目前在預覽中的功能被以 **（預覽）**。

> [!TIP]
> 沒有 Office 365 ATP 尚未？ [若要啟動試用版的連絡人銷售](https://go.microsoft.com/fwlink/p/?LinkId=518644)。

## <a name="januaryfebruary-2020"></a>年 1 月/年 2 月 2020

- 若要啟用安全性作業小組來搜尋及篩選多個欄位的[威脅總管](threat-explorer.md)的增強功能時[調查的電子郵件](investigate-malicious-email-that-was-delivered.md)：
    - 傳遞位置和特殊的動作
    - 方向 (輸入、 輸出，或組織內部)
    - 會覆寫 （封鎖或允許的使用者）
    - URL 威脅 （惡意程式碼、 釣魚程式、 垃圾郵件，或無）
    - 進階不篩選 (這些進階篩選選項，包括不包含，不包含，等等。)
    - 細微時間篩選 （一天、 hour、 半） 

- [**事件**] 小工具現在是 [**重要訊息中心**] 小工具。 (若要檢視您的安全性 widget，在 Office 365 安全性 & 合規性中心，移至 [**威脅管理** > **檢閱**。)

- [在 Office 365 中的安全文件](https://docs.microsoft.com/microsoft-365/security/office-365-security/safe-docs) **（預覽）**

## <a name="december-2019"></a>2019 年 12 月

- [匯出的 URL，按一下 [離線分析的資料](threat-explorer.md#new-features-in-threat-explorer-and-real-time-detections)（Office 365 ATP 方案 1 或計劃 2）

- [使用 Office 365 ATP (**預覽**) 中的活動檢視](campaigns.md)

## <a name="november-2019"></a>2019 年 11 月

- [嗎？ 請查看新的使用者遭入侵的偵測及回應功能](address-compromised-users-quickly.md)(**預覽**) (Office 365 ATP 計劃 2)

## <a name="september-2019"></a>2019 年 9 月

- [雇用自動化的調查和 Office 365 中的回應功能](automated-investigation-response-office.md)(Office 365 ATP 計劃 2)

- [Office 365 ATP 整合項目自動進行調查並使用 Office 365 管理活動 API 回應事件](https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-schema#office-365-advanced-threat-protection-and-threat-investigation-and-response-schema)(Office 365 ATP 計劃 2)

- [檢視的電子郵件標頭，並下載電子郵件內文](investigate-malicious-email-that-was-delivered.md)（Office 365 ATP 方案 1 或計劃 2）

## <a name="august-2019"></a>2019 年 8 月

- [電子郵件的時間表] 檢視](investigate-malicious-email-that-was-delivered.md#view-the-timeline-of-your-email)（Office 365 ATP 方案 1 或計劃 2）

## <a name="july-2019"></a>2019 年 7 月

- [檢查傳遞巨集指令與電子郵件的位置](investigate-malicious-email-that-was-delivered.md#check-the-delivery-action-and-location)（Office 365 ATP 計劃 1 或 2）

## <a name="june-2019"></a>2019 年 6 月

- [檢視網路釣魚 Url 相關資料，並按一下 verdict](threat-explorer.md#view-data-about-phishing-urls-and-click-verdict) （Office 365 ATP 方案 1 或計劃 2）

## <a name="office-365-atp-plan-1-and-plan-2"></a>Office 365 ATP 方案 1 與方案 2

您知道 Office 365 ATP 可用於兩個計劃嗎？ [解更多關於每個方案所包含的內容](office-365-atp.md#office-365-atp-plan-1-and-plan-2)。

## <a name="see-also"></a>另請參閱

[Microsoft 365 藍圖](https://www.microsoft.com/microsoft-365/roadmap)

[Office 365 ATP 服務說明](https://docs.microsoft.com/office365/servicedescriptions/office-365-advanced-threat-protection-service-description)

[Microsoft 威脅防護的自動化調查及回應](https://docs.microsoft.com/microsoft-365/security/mtp/mtp-autoir)
