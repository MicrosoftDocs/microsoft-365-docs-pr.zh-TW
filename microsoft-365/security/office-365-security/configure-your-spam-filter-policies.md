...
1. 在安全性中心，移至 [電子郵件與共同作業 **]** \> [原則與規則 **]** \> [威脅原則 **]** \> [反垃圾郵件 **]**。

2. 按一下 ![建立圖示](../../media/m365-cc-sc-create-icon.png) [建立原則 **]**，然後從下拉式清單選取 [輸入 **]**。
...
5. 在 [大量電子郵件閾值與垃圾郵件屬性 **]** 頁面上，設定下列設定：
   ...
   - **包含特定語言**：按一下方塊，然後從下拉式清單選取 [開啟 **]** 或 [關閉 **]**。 如果您將它開啟，會顯示一個方塊。 開始在方塊中輸入語言的名稱。 已篩選的支援語言清單將會顯示。 當您找到所需語言時，請選取該語言。 視需要重複此步驟多次。 若要移除現有的值，請按一下值旁邊的 ![移除圖示](../../media/m365-cc-sc-remove-selection-icon.png)。

   - **來自這些國家/地區** _：按一下方塊，然後從下拉式清單選取 [開啟_ *]** 或 [關閉 **]**。 如果您將它開啟，會顯示一個方塊。 開始在方塊中輸入國家/地區的名稱。 已篩選的支援國家/地區清單將會顯示。 當您找到您要尋找的國家/地區時，請選取它。 視需要重複此步驟多次。 若要移除現有的值，請按一下值旁邊的 ![移除圖示](../../media/m365-cc-sc-remove-selection-icon.png)。
...
## <a name="use-the-security-center-to-remove-anti-spam-policies"></a>使用安全性中心來移除反垃圾郵件原則

使用安全性中心來移除反垃圾郵件原則時，會同時刪除垃圾郵件篩選規則與對應的垃圾郵件篩選原則。 您無法移除預設原則。
