...
在您為特權帳戶新增額外的保護，並防範已知的攻擊後，請轉向您的注意，以防範未知的威脅。 更確定和高級的敵人使用創新及新的未知方法來攻擊組織。 透過使用 Microsoft 豐富的資料遙測功能（透過十億種裝置、應用程式和服務來收集），我們可以在 Windows、Office 365 和 Azure 上執行 Office 365 的 Defender，以避免 Zero-Day 攻擊、利用沙灘 box 環境，以及檢查有效性，再允許存取您的內容。 


|建議  |E3  |E5  |
|---------|---------|---------|
|**設定 Microsoft Defender For Office 365**：<br>* 安全附件<br>* 安全連結<br>* 適用于 SharePoint、OneDrive 和 Microsoft 小組的 microsoft Defender for Endpoint<br>* Office 365 的 Defender 中的反網路釣魚保護|         |![綠色核取記號](../media/green-check-mark.png) |
...
- 封鎖所有非管理員對 Microsoft Azure 管理入口網站的存取權。 若要達到此目的，您可以建立一個條件式存取規則，以封鎖除系統管理員以外的所有使用者。 
...
|建議 |E3|E5 |
|---------|---------|---------|
|**檢查和優化您的條件式存取和相關原則，以符合零信任網路的目標**。 防範已知威脅包括實施一組建議的 [原則](./office-365-security/microsoft-365-policies-configurations.md)。 請複查這些原則的實施，以確保您保護應用程式和資料，以防範已取得您網路存取權的駭客。 建議使用 Windows 10 的 Intune 應用程式保護原則，啟用 Windows 資訊保護 (WIP) 。 WIP 可防止透過應用程式和服務（例如電子郵件、社交媒體和公用雲端）意外洩漏組織資料。 |         |![綠色核取記號](../media/green-check-mark.png)|
...
- **每週查看主要惡意程式碼和 phished 使用者** —使用 Microsoft Defender for Office 365 威脅瀏覽器來查看主要具有惡意程式碼和網路釣魚的使用者，並找出這些使用者受到影響的根本原因。