...
Microsoft 會在您的租使用者中提供安全評分工具，以根據您的定期活動自動分析安全性狀態、指派分數，並提供安全性改進建議。 在執行本文中建議的動作之前，請注意您目前的分數和建議。 本文中建議的動作可增加您的分數。 目標不會達到最大分數，但若要瞭解如何保護您的環境，請務必以不會對使用者生產力造成負面影響的方式來保護您的環境。 請參閱 [Microsoft 安全分數](defender/microsoft-secure-score.md)。
...
在您為特權帳戶新增額外的保護，並防範已知的攻擊後，請轉向您的注意，以防範未知的威脅。 更確定和高級的敵人使用創新及新的未知方法來攻擊組織。 透過使用 Microsoft 成千上萬的資料遙測功能，透過數十億美元的裝置、應用程式和服務來收集，我們可以在 Windows、Office 365 和 Azure 上執行 Office 365 的 Defender，以防止零天的攻擊、利用沙灘 box 環境，以及檢查有效性，再允許存取您的內容。 
