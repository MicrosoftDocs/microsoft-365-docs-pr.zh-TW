...
## <a name="is-attack-surface-reduction-asr-part-of-windows"></a> (ASR) Windows 的一部分是否減少攻擊面？
...
只有在您有 Windows 10 企業版授權時，才支援完整的 ASR 規則和功能集。 在沒有企業版授權的情況下，有限的規則數目可以運作。 如果您有 Microsoft 365 商務版，請將 Microsoft Defender 防毒軟體設為主要的安全性解決方案，並透過 PowerShell 啟用規則。 未正式支援使用沒有企業版授權的 ASR，而且您將無法使用 ASR 的完整功能。
...
建議您啟用每一種可能的規則。 不過，在某些情況下，您不應啟用規則。 例如，如果您使用 Microsoft Endpoint Configuration Manager (或是，System Center Configuration Manager-SCCM) 來管理您的端點，我們不建議啟用來自 PSExec 和 WMI 命令規則的封鎖處理常式建立。
...
## <a name="i-applied-a-rule-using-gpo-now-when-i-try-to-check-the-indexing-options-for-the-rule-in-microsoft-outlook-i-get-a-message-stating-access-denied"></a>我使用 GPO 套用規則。 現在當我嘗試檢查 Microsoft Outlook 中的規則的索引選項時，收到一則訊息，說明「拒絕存取」。
...
## <a name="i-recently-enabled-the-asr-rule-block-credential-stealing-from-the-windows-local-security-authority-subsystem-lsassexe-and-i-am-getting-a-large-number-of-notifications-what-is-going-on"></a>我最近啟用了 ASR 規則，*封鎖從 Windows 本機安全授權子系統 (lsass.exe) 中偷竊的認證*，而且我收到大量通知。 這是怎麼回事？
