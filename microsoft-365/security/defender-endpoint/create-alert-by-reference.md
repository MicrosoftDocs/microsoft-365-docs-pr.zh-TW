...
<br>需要有 **Microsoft Defender For Endpoint 事件** 才能建立警示。
<br>您必須從要求中的事件提供3個參數： **事件時間**、 **電腦識別碼** 及 **報表識別碼**。 請參閱下面範例。
<br>您可以使用在高級搜尋 API 或入口網站中找到的事件。
<br>如果在相同的裝置上有相同標題的開啟警示，新建立的警示會與其合併。
<br>自動調查會在透過 API 建立的提醒上自動啟動。
...
reportId | 字串 | 從高級搜尋取得的事件 reportId。 **必要欄位**。
machineId | 字串 | 識別事件之裝置的識別碼。 **必要欄位**。
嚴重性 | 字串 | 警示的嚴重性。 屬性值為：「低 '、' 中」及 ' 高」。 **必要欄位**。
標題 | String | 提醒的標題。 **必要欄位**。
描述 | 字串 | 警示的描述。 **必要欄位**。
recommendedAction| 字串 | 在分析警示時，安全性監察官建議採取的動作。 **必要欄位**。
