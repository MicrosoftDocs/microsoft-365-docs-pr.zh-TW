...
方法 | 資料類型 | 描述
:---|:---|:---
[匯出 secure configuration 評估 (OData) ](get-assessmnt-secure-cfg.md#1-export-secure-configuration-assessment-odata) | 依裝置集合的安全設定。 請參閱： [1.2 屬性 (OData) ](#12-properties-odata) | 會傳回資料表，其中包含對應之 DeviceId，ConfigurationId 的每個唯一組合的專案。  (_OData_ _或透過_ 檔案所收集的資料，) 是目前狀態的目前快照，而且不包含歷史資料。 為了收集歷史資料，客戶必須將資料儲存在自己的資料儲存中。
[透過檔案匯出 secure configuration 評估 () ](get-assessmnt-secure-cfg.md#2-export-secure-configuration-assessment-via-files) | 依裝置檔案的安全設定。 請參閱：1.3 透過檔案 [ (的屬性) ](#13-properties-via-files) | 會傳回資料表，其中包含對應之 DeviceId，ConfigurationId 的每個唯一組合的專案。 此 API 解決方案可讓大量的資料更快速且可靠地進行。 因此，建議大型組織使用超過10個裝置。 此 API 會將組織中的所有資料都提取為下載檔案。 回應包含從 Azure 儲存體下載所有資料的 URLs。 此 API 可讓您從 Azure 儲存體下載所有資料，如下所示：1。  呼叫 API 以取得所有組織資料的下載 URLs 清單。 2.  使用下載 URLs 下載所有檔案，並視需要處理資料。
...
方法 | 資料類型 | 描述
:---|:---|:---
[匯出軟體清查評估 (OData) ](get-assessmnt-software-inventory.md#1-export-software-inventory-assessment-odata) | 依裝置集合的軟體清查。 請參閱： [2.2 屬性 (OData) ](#22-properties-odata) | 會傳回資料表，其中包含 DeviceId、SoftwareVendor、SoftwareName、SoftwareVersion 的每個唯一組合的專案。  (_OData_ _或透過_ 檔案所收集的資料，) 是目前狀態的目前快照，而且不包含歷史資料。 為了收集歷史資料，客戶必須將資料儲存在自己的資料儲存中。
[透過檔案匯出軟體清查評估 () ](get-assessmnt-software-inventory.md#2-export-software-inventory-assessment-via-files) | 依裝置檔案清點軟體。 請參閱：2.3 透過檔案 [ (的屬性) ](#23-properties-via-files) | 會傳回資料表，其中包含 DeviceId、SoftwareVendor、SoftwareName、SoftwareVersion 的每個唯一組合的專案。 此 API 解決方案可讓大量的資料更快速且可靠地進行。 因此，建議大型組織使用超過10個裝置。 此 API 會將組織中的所有資料都提取為下載檔案。 回應包含從 Azure 儲存體下載所有資料的 URLs。 此 API 可讓您從 Azure 儲存體下載所有資料，如下所示：1。  呼叫 API 以取得所有組織資料的下載 URLs 清單。 2.  使用下載 URLs 下載所有檔案，並視需要處理資料。
...
## <a name="3-export-software-vulnerabilities-assessment-per-device"></a>3. 匯出每個裝置的軟體漏洞評估
...
方法 | 資料類型 | 描述
:---|:---|:---
[匯出軟體漏洞評估 (OData) ](get-assessmnt-software-vulnerabilities.md#1-export-software-vulnerabilities-assessment-odata) | 調查集合請參閱： [3.2 屬性 (OData) ](#32-properties-odata) | 會傳回資料表，其中包含 DeviceId、SoftwareVendor、SoftwareName、SoftwareVersion、CveId 的每個唯一組合的專案。  (_OData_ _或透過_ 檔案所收集的資料，) 是目前狀態的目前快照，而且不包含歷史資料。 為了收集歷史資料，客戶必須將資料儲存在自己的資料儲存中。
[透過檔案匯出軟體漏洞評估 () ](get-assessmnt-software-vulnerabilities.md#2-export-software-vulnerabilities-assessment-via-files) | 調查實體請參閱：3.3 透過檔案 [ (的屬性) ](#33-properties-via-files) | 會傳回資料表，其中包含 DeviceId、SoftwareVendor、SoftwareName、SoftwareVersion、CveId 的每個唯一組合的專案。 此 API 解決方案可讓大量的資料更快速且可靠地進行。 因此，建議大型組織使用超過10個裝置。 此 API 會將組織中的所有資料都提取為下載檔案。 回應包含從 Azure 儲存體下載所有資料的 URLs。 此 API 可讓您從 Azure 儲存體下載所有資料，如下所示：1。  呼叫 API 以取得所有組織資料的下載 URLs 清單。 2.  使用下載 URLs 下載所有檔案，並視需要處理資料。
...
- [每個裝置的匯出軟體漏洞評估](get-assessmnt-software-vulnerabilities.md)
