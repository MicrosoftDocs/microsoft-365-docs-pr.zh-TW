...
3. 在結果清單中的 [ **AMRunningMode** ] 列中，尋找下列其中一個值：
   - `Normal` -Defender 服務正常運作。 未啟用特殊模式。
   - `Passive Mode`-如果您的組織使用 Microsoft Defender for Endpoint 搭配非 Microsoft 防病毒/反惡意程式碼解決方案，則 Microsoft Defender 防毒軟體會自動進入被動模式。 Microsoft Defender 防毒軟體不會修正 (即時保護和威脅。 ) 
   - `SxS Passive Mode`-類似于被動模式，但具有開啟有限定期掃描的選項。
