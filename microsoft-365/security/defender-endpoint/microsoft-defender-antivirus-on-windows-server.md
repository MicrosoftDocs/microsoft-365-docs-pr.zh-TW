...
- 在 Windows Server 上，如果您正在執行非 Microsoft 防病毒/反惡意程式碼解決方案，則 Microsoft Defender 防毒程式不會自動進入被動模式或停用模式。 不過，您可以手動將 Microsoft Defender 防毒軟體設定為被動或 disabled 模式。

## <a name="setting-up-microsoft-defender-antivirus-on-windows-server"></a>設定 Windows Server 上的 Microsoft Defender 防毒軟體
...
根據預設，Windows Server 上已安裝 Microsoft Defender 防毒軟體，且運作正常。 在預設情況下，使用者介面 (GUI) 會預設安裝，但不需要 GUI。 您可以使用 PowerShell、群組原則或其他方法來管理 Microsoft Defender 防病毒。 
...
如果您需要在 Windows Server 上安裝或重新安裝 Microsoft Defender 防病毒，您可以使用 [ **新增角色及功能] 嚮導** 或 PowerShell 來執行。

### <a name="use-the-add-roles-and-features-wizard-to-install-microsoft-defender-antivirus"></a>使用 [新增角色及功能] 嚮導來安裝 Microsoft Defender 防毒程式
...
### <a name="use-powershell-to-install-microsoft-defender-antivirus"></a>使用 PowerShell 安裝 Microsoft Defender 防毒程式
...
安裝 Microsoft Defender 防病毒之後，下一步是驗證它是否正在執行。 在 Windows Server 端點上，執行下列 PowerShell Cmdlet：
...
範例提交可讓 Microsoft 收集可能惡意軟體的範例。 為了協助提供持續且最新的保護，Microsoft 研究員使用這些範例來分析可疑活動，並產生更新的反惡意軟體安全性情報。 我們會收集程式的可執行檔，例如 .exe 檔案及 .dll 檔案。 我們不會收集包含個人資料的檔案，像是 Microsoft Word 檔和 PDF 檔案。
...
- 在 Windows Server 上，版本1803或更新版本，或 Windows Server 2019，您可以將 Microsoft Defender 防毒軟體設定為被動模式。  

- 在 Windows Server 2016 上，非 Microsoft 防病毒/反惡意軟體產品不支援 Microsoft Defender 防毒軟體。 在這些情況下，您必須將 Microsoft Defender 防毒軟體設定為停用模式。

### <a name="set-microsoft-defender-antivirus-to-passive-mode-using-powershell"></a>使用 PowerShell 將 Microsoft Defender 防毒程式設定為被動模式

如果您使用的是 Windows Server、版本1803或 Windows Server 2019，您可以使用下列 PowerShell Cmdlet，將 Microsoft Defender 防毒軟體設定為被動式模式：
...
若要在 Windows Server 2016 上停用 Microsoft Defender 防病毒，請使用下列 PowerShell Cmdlet：
