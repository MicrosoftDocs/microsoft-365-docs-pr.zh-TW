...
Defender for Endpoint 使用 Windows 10 和 Microsoft 強健雲端服務內建的下列技術組合：

-   **端點行為感應** 器：內嵌在 Windows 10 中，這些感應器會從作業系統收集並處理行為信號，並將此感應器資料傳送至 Microsoft Defender for Endpoint 的私人、隔離的雲端實例。


-   **雲端安全性分析**：跨 Windows 生態系統利用大量資料、裝置教學和獨特的 Microsoft 光纖，企業雲端產品 (例如 Office 365) 和線上資產）都會將行為信號轉譯成深入、偵測及建議的高級威脅回應。
...
>[!IMPORTANT]
>Defender for Endpoint customers 客戶必須套用 Microsoft 威脅專家 managed 威脅搜尋服務，以取得主動目標的攻擊通知，並與專家合作。 按需分配的專家是附加元件服務。 當您已接受 Microsoft 威脅專家管理的威脅搜尋服務後，系統將永遠包含目標攻擊通知。<p>
><p>若尚未註冊，且想要體驗其優點，請移至 [<b>設定</b> > <b>] [一般</b>> <b>高級功能</b>] > <b>Microsoft 威脅專家</b>適用。 一旦接受，您就會獲得目標攻擊通知的優點，並在需要時啟動90天的專家試用。 請與您的 Microsoft 代表聯繫，以取得對需求訂閱的完整專家。
...
使用 Microsoft 365 Defender、Defender for Endpoint 和各種 Microsoft security 解決方案，可在本機整合的所有端點、身分識別、電子郵件和應用程式，形成整合的前期和後續的企業防護套件，以偵測、避免、調查和自動回應複雜的攻擊。
