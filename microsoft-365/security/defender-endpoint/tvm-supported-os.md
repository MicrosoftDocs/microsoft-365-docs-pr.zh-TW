...
開始之前，請先確定您符合下列威脅與弱點管理的作業系統或平臺必要條件，讓裝置中的活動正確地加以考慮。

>[!NOTE]
>支援的威脅與弱點管理系統和平臺可能不同于[Microsoft Defender for Endpoint list 的最低需求](minimum-requirements.md)。

## <a name="capabilities-per-supported-operating-systems-os-and-platforms"></a>每個受支援作業系統的功能 (OS) 與平臺

在下表中，「是」表示該資料列的作業系統或平臺支援威脅與弱點管理功能。

支援的作業系統或平臺 | 作業系統弱點 | 軟體產品弱點 | 作業系統設定評估 | 安全性控制設定評估 | 軟體產品設定評估
:---|:---|:---|:---|:---|:---
Windows 7 | 是 | 不支援 | 不支援 | 不支援 | 不支援
Windows 8.1 | 是 | 是 | 是 | 是| 是
Windows 10，版本1607-1703 | 是  | 不支援 | 不支援 | 不支援 | 不支援
Windows 10 版本 1709 或更新版本。 | 是 | 是 | 是 | 是 | 是
...
macOS 10.14 "Mojave" 以上 | 是 | 是 | 是 | 是 | 是 
Red Hat Enterprise Linux 7.2 或更高版本 (\* 請參閱以下的「重要」注意)  | 是 | 是 | 是 | 是 | 是
...
>[!NOTE]
> 有些功能不適用於下層作業系統，請參閱 Microsoft 365 Defender 入口網站以取得支援作業系統的詳細資料。
