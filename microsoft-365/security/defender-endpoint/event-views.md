...
如果您有 E5 訂閱並使用 [Microsoft Defender For Endpoint](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/microsoft-defender-advanced-threat-protection)，請以 Windows 安全性的方式深入報告事件和區塊。
...
1. 建立空白的 .txt 檔案，並將您想要使用的自訂視圖的 XML 複製到 .txt 檔案中。 針對每個您想要使用的自訂視圖執行這項作業。 將檔案重新命名如下 (確定您將類型從 .txt 變更為 .xml) ：
...
2. 展開 [ **應用程式及服務] 記錄 > Microsoft > Windows** ]，然後移至下表中 [ **提供者/來源** ] 底下所列的資料夾。
...
受控資料夾存取權 | Windows Defender (運作)  | 5007 | 設定變更時的事件
受控資料夾存取權 | Windows Defender (運作)  | 1124 | 已審核的受管理資料夾存取事件
