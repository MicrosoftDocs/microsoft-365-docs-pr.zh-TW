...
> [!NOTE]
> 在上架至 Defender for Endpoint 之後，您可能必須將 Windows 伺服器上的 Microsoft Defender 防毒軟體設定為被動模式。 若要驗證被動式模式已設定為預期，請在 **Microsoft Windows Windows Defender 操作** 記錄檔)  (中搜尋 *事件 5007* ， `C:\Windows\System32\winevt\Logs` 並確認 **ForcePassiveMode** 或 **PassiveMode** 登錄機碼已設定為 **0x1**。
