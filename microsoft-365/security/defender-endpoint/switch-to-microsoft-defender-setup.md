...
在某些 Windows 版本（如 Windows Server）上，當您安裝 McAfee 解決方案時，Microsoft Defender 防毒程式可能已卸載或停用。 當您準備好將端點架到 Defender for Endpoint 時，Microsoft Defender 防毒程式不會自動進入被動式或 disabled 模式。 此外，在 Windows Server 上，您無法在使用中的模式與非 Microsoft 防毒軟體/反惡意軟體（例如 McAfee、Symantec 或其他使用者）的 active 模式中擁有 Microsoft Defender 防毒軟體。 若要深入瞭解使用 Defender for Endpoint 和防病毒解決方案所發生的情況，請參閱 [Microsoft Defender 防毒程式相容性](/windows/security/threat-protection/microsoft-defender-antivirus/microsoft-defender-antivirus-compatibility)。

若要協助確保已啟用 Microsoft Defender 防病毒，並以被動式模式進行，請完成本文所述的下列工作：
...
過去使用的 [DisableAntiSpyware](/windows-hardware/customize/desktop/unattend/security-malware-windows-defender-disableantispyware) 登錄機碼停用 Microsoft Defender 防病毒，並部署其他防病毒產品（例如 McAfee）。 一般說來，您的 Windows 裝置和端點上不應該使用此登錄機碼，不過，如果您已 `DisableAntiSpyware` 設定，以下是將其值設為 false 的方式：
...
> - Windows Server 2016 (查看 [您使用 Windows Server 2016 的重要資訊嗎？](#are-you-using-windows-server-2016)) 
...
如果您有執行 Windows Server 2016 的端點，您就無法在非 Microsoft 防病毒/反惡意程式碼解決方案旁邊執行 Microsoft Defender 防毒程式。 Microsoft Defender 防病毒無法在 Windows Server 2016 上以被動模式執行。 在此情況下，您將需要卸載非 Microsoft 防病毒/反惡意程式碼解決方案，然後改為安裝/啟用 Microsoft Defender 防病毒。 若要深入瞭解，請參閱 [防病毒解決方案與 Defender For Endpoint 的相容性](microsoft-defender-antivirus-compatibility.md)。
...
> [!TIP]
> 仍需要協助嗎？ 請參閱 [Windows Server 上的 Microsoft Defender 防病毒](microsoft-defender-antivirus-on-windows-server.md)。
...
> [!IMPORTANT]
> 您可以在 Windows Server、版本1803或更新版本，或 Windows Server 2019 上，將 Microsoft Defender 防毒軟體設定為被動模式。 但是 Windows Server 2016 不支援被動模式。 若要深入瞭解，請參閱 [防病毒解決方案與 Microsoft Defender For Endpoint 的相容性](defender-compatibility.md)。
...
|Windows 中的控制台     |遵循下列指導： [開啟 Microsoft Defender 防毒軟體](/mem/intune/user-help/turn-on-defender-windows)。 <p>**附注**：在某些 windows 版本中，您可能會看到 *Windows defender 防病毒* ，而不是 *Microsoft Defender 防毒軟體* 。        |
|[高級群組原則管理](/microsoft-desktop-optimization-pack/agpm/) <br/>或<br/>[群組原則管理主控台](/windows/security/threat-protection/microsoft-defender-antivirus/use-group-policy-microsoft-defender-antivirus)  | 1. 移至 **電腦設定系統**  >  **管理範本**  >  **Windows 元件**  >  **Microsoft Defender 防毒軟體**。 <p> 2. 尋找稱為關閉 **Microsoft Defender 防病毒** 程式的原則。<p> 3. 選擇 [ **編輯原則設定**]，並確定已停用原則。 此巨集指令可啟用 Microsoft Defender 防毒軟體。 <p>**附注**：在某些 windows 版本中，您可能會看到 *Windows defender 防病毒* ，而不是 *Microsoft Defender 防毒軟體* 。 |
...
要設定的特定排除專案將取決於您的端點或裝置執行所在的 Windows 版本，以及下表所列的專案：
...
## <a name="add-your-existing-solution-to-the-exclusion-list-for-microsoft-defender-antivirus"></a>將現有的解決方案新增至 Microsoft Defender 防病毒的排除清單

在安裝程式的這個步驟中，您可以將現有的解決方案新增至 Microsoft Defender 防病毒排除清單。 
...
|[群組原則物件](/previous-versions/windows/desktop/Policy/group-policy-objects) | 1. 在您的群組原則管理電腦上，開啟「 [群組原則管理主控台](https://technet.microsoft.com/library/cc731212.aspx)」，以滑鼠右鍵按一下您要設定的群組原則物件，然後選取 [ **編輯**]。<p> 2. 在 [**群組原則管理編輯器**] 中，移至 [電腦設定]，然後選取 [**系統****管理範本**]。<p> 3. 展開樹狀目錄至 **Windows 元件 > Microsoft Defender 防病毒 > 排除**。<br/>**附注**：在某些 windows 版本中，您可能會看到 *Windows defender 防病毒* ，而不是 *Microsoft Defender 防毒軟體* 。<p> 4. 按兩下 [ **路徑排除** ] 設定，並新增排除。<br/>-將選項設定為 [ **啟用**]。<br/>-在 [ **選項** ] 區段下，選取 [ **顯示 ...**]。<br/>-在 [ **值名稱** ] 欄底下，指定各自列的每一個資料夾。<br/>-如果您指定的是檔案，請務必輸入檔案的完整路徑，包含磁碟機號、資料夾路徑、檔案名及副檔名。 在 [**值**] 欄中輸入 **0** 。<p> 5. 選取 **[確定]**。<p> 6. 按兩下 [ **副檔名排除** ] 設定，並新增排除。<br/>-將選項設定為 [ **啟用**]。<br/>-在 [ **選項** ] 區段下，選取 [ **顯示 ...**]。<br/>-在 [ **值名稱** ] 欄位底下的獨佔行中輸入每個副檔名。  在 [**值**] 欄中輸入 **0** 。<p> 7. 選取 **[確定]**。 |
|本機組策略物件 |1. 在端點或裝置上，開啟 [本機群組原則編輯器]。 <p>2. 移至 [電腦設定]**系統**  >  **管理範本**  >  **Windows 元件**  >  **Microsoft Defender 防病毒**  >  **排除**。<p>**附注**：在某些 windows 版本中，您可能會看到 *Windows defender 防病毒* ，而不是 *Microsoft Defender 防毒軟體* 。<p>3. 指定您的路徑與處理常式排除。 |
