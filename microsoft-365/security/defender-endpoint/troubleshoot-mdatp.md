...
本節說明當您使用 Microsoft Defender for Endpoint service 時可能會發生的問題。
...
如果 Microsoft Defender 資訊安全中心上的某些元素或資料遺失，可能是因為 proxy 設定封鎖它。
...
如果上架裝置順利完成，但在重新開機後，Microsoft Defender for Endpoint 不會啟動，並顯示錯誤577，請檢查原則未停用 Windows Defender。

如需詳細資訊，請參閱[確認原則中未停用 Microsoft Defender 防毒軟體](troubleshoot-onboarding.md#ensure-that-microsoft-defender-antivirus-is-not-disabled-by-a-policy)。
