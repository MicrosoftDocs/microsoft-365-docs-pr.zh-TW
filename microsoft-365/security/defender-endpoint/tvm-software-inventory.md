...
---
# <a name="software-inventory---threat-and-vulnerability-management"></a>軟體清查-威脅與弱點管理
...
威脅與弱點管理中的軟體清查是組織中具有官方[通用平臺列舉 (CPE) ](https://nvd.nist.gov/products/cpe)的已知軟體清單。 沒有官方 CPE 的軟體產品不會發佈弱點。 此外，它也包含廠商名稱、弱點數目、威脅及公開裝置數目等詳細資訊。
...
從 [Microsoft Defender 資訊安全中心](portal-overview.md)的 [威脅與弱點管理導覽] 功能表中選取 [**軟體清查**]，即可存取軟體庫存頁面。
...
威脅 & 弱點管理目前不支援的軟體可能會出現在 [軟體清單] 頁面中。 由於不支援，因此只能使用有限的資料。 在「弱點」區段中，使用不支援的軟體篩選。
...
從 Microsoft Defender 資訊安全中心導覽面板中，移至 [**[裝置] 清單](machines-view-overview.md)**。 選取裝置的名稱以開啟 [裝置] 頁面 (例如 Computer1) ]，然後選取 [ **軟體庫存** ] 索引標籤，以查看裝置上存在的所有已知軟體清單。 選取特定的軟體專案，以使用詳細資訊開啟快顯視窗。

即使目前不威脅與弱點管理支援軟體，也可以在裝置層級看到軟體。 不過，只有有限的資料可供使用。 您會知道軟體是否不受支援，因為這會在「弱點」欄中說「無法使用」。
...
- [事件時程表頁面](threat-and-vuln-mgt-event-timeline.md)> 選取事件 > 在快顯視窗中，選取名為 "相關元件" 的區段中的超連結軟體名稱 (（如 Visual Studio 2017) ）。
