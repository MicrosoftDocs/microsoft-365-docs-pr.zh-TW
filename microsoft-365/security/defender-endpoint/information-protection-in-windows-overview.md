...
資訊保護是 Microsoft 365 企業版套件不可或缺的一部分，提供智慧防護以保持機密資料的安全性，同時在工作場所中啟用生產力。
...
- **資料分類**-根據常見的 Microsoft 資訊保護 (MIP) 原則，以 Office 365 安全性 & 規範中心管理。 自動分類可讓您保護機密資料，即使使用者未手動分類也是如此。
...
Office 365 資料遺失防護中的機密資訊類型 (DLP) 實現分為兩類：

- 預設
- 自訂

預設的機密資訊類型包括銀行帳戶號碼、社會保險號碼或國內 IDs 等資訊。 如需詳細資訊，請參閱 [敏感資訊類型的外觀](/office365/securitycompliance/what-the-sensitive-information-types-look-for)。

自訂類型是您定義的，其設計目的是為了保護不同類型的敏感資訊 (例如，員工 IDs 或專案編號) 。 如需詳細資訊，請參閱 [建立自訂機密資訊類型](/office365/securitycompliance/create-a-custom-sensitive-information-type)。
...
您也可以在 [Azure 記錄分析](/azure/log-analytics/log-analytics-overview)中使用以 Defender 做為基礎的資料探索，您可以在原始資料上執行複雜的查詢。

如需 Azure 資訊保護分析的詳細資訊，請參閱 [適用于 Azure 資訊保護的中央報告](/azure/information-protection/reports-aip)。
...
- 在 Microsoft Defender 資訊安全中心中啟用 Azure 資訊保護整合：
    - 移至 **設定** 在 Microsoft Defender 資訊安全中心中，按一下 **[一般**] 下的 [**高級] 設定**。
