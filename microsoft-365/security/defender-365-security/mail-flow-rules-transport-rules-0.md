...
- **屬性**：指定其他不是條件、例外狀況或動作的規則設定。 例如，何時應套用規則、是否強制執行或測試規則，以及規則作用中的時間週期。
  ...
  如需詳細資訊，請參閱本文中的 [郵件流程規則屬性](#mail-flow-rule-properties) 一節。
...
通過組織中的所有郵件都會根據組織已啟用的郵件流程規則來評估。 規則會以 EAC 中的 [**郵件流程** 規則] 頁面上所列的連續處理 \>  ，或是根據 PowerShell 中的對應 _優先順序_ 參數值進行處理。
