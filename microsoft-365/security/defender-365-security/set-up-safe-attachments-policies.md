...
- 如需安全附件原則的建議設定，請參閱 [安全附件設定](recommended-settings-for-eop-and-office365.md#safe-attachments-settings)。
...
4. 在顯示的 [ **設定** ] 頁面上，設定下列設定：
   ...
   - **安全附件未知的惡意程式碼回應**：選取下列其中一個值：
     ...
     這些值會在 [安全附件原則設定](safe-attachments.md#safe-attachments-policy-settings)中說明。
