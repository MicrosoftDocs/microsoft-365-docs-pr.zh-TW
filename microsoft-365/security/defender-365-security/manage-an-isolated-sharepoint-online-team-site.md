...
- [適用於 Office 365 的 Microsoft Defender 方案 1](defender-for-office-365.md)
- SharePoint Online 
...
如果您是透過 Active Directory Domain Services (AD DS) 來管理使用者帳戶和群組，請使用一般 AD DS 使用者和群組管理程式將適當的使用者新增至適當的訪問群組，並等待與您的訂閱同步處理。
...
- 在 PowerShell 中，先 [與 Azure Active Directory PowerShell for Graph 模組連線](../../enterprise/connect-to-microsoft-365-powershell.md#connect-with-the-azure-active-directory-powershell-for-graph-module)。 若要將使用者帳戶新增至具有使用者主要名稱 (UPN) 的 access 群組，請使用下列 PowerShell 命令區塊：
...
15. 在左側導覽中的 [**群組**] 底下，按一下 [ **\<site name> 訪客**] 群組，並使用步驟11-14，根據需要在子資料夾中視需要查看檔案 (，以指定使用者帳戶的集合) 。

16. 在左側導覽中的 [**群組**] 底下，按一下 [ **\<site name> 擁有** 者] 群組，並使用步驟11-14，根據需要在子資料夾中指定許可權，以指定使用者帳戶的組 () 。
...
[為團隊設定安全性隔離](/microsoft-365/solutions/secure-teams-security-isolation)
