...
---
# <a name="microsoft-secure-score-preview"></a> (預覽的 Microsoft 安全分數) 
...
每個改進動作都值得10點或更少。 大多數會以二進位方式計分：若您執行改進動作（如建立新原則或開啟特定設定），您會收到100% 的點數。 在其他改進動作中，點會指定為總設定的百分比。 例如，如果改進的動作指出您使用多重要素驗證來保護所有使用者時取得30點，而且您只會保護 100 5% 以上的使用者，則會得到大約2點的部分分數 (5 protected/100 總計 * 30 最大值 = 2 pt 部分分數) 。
...
* SecurityEvents 唯讀角色的所有 () 
* ReadWrite SecurityEvents (讀取和寫入角色的所有) 
...
>[!NOTE]
>在最近發行的 Microsoft Secure 得分中，已發行的評分模型已發佈，使 Microsoft 安全分數暫時不相容身分識別安全分數和圖形 API。 [檢視詳細資料](microsoft-secure-score-preview.md#incompatibility-with-identity-secure-score-and-graph-api)
...
- 不允許使用者授與未受管理的應用程式同意 (目前可用於發行版本本) 
...
#### <a name="support-for-microsoft-defender-atp-threat--vulnerability-management-tvm-security-recommendations"></a>支援 Microsoft Defender ATP 威脅 & 弱點管理 (TVM) 安全性建議
