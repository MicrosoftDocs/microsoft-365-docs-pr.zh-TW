...
| `Issuer` | string | 發證憑證授權單位（CA）的相關資訊 |
| `IssuerHash` | string | 識別發證憑證授權單位（CA）的唯一雜湊值 |
| `CertificateSerialNumber` | string | 發證憑證授權單位（CA）獨有之憑證的識別碼 |
| `CrlDistributionPointUrls` | string |  列出包含憑證和憑證吊銷清單（Crl）之網路共用 URLs 的 JSON 陣列 |
