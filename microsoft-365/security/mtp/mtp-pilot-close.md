...
您剛剛模擬的是僅限記憶體的高級攻擊，可在網域控制站上遠端執行程式碼。 您已瞭解 Microsoft Defender ATP 和 Azure ATP 如何偵測及警示 stealthy 惡意活動。 您也瞭解如何將不同來源的警示傳遞給 Microsoft 威脅防護入口網站中的單一事件，讓 SOC 分析人員能夠調查並採取必要的動作。 您也已經建立高級搜尋查詢，識別輸入電子郵件，使用者已開啟或儲存附件，並根據該查詢建立偵測。
