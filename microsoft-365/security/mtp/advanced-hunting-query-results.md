...
下列查詢會使用一個`join`運算子從`DeviceInfo`資料表中拉入 OS 資訊，然後使用`summarize`來計數和`OSPlatform` `Severity`欄中的值：
...
![高級搜尋查詢結果的圖像顯示為餅形圖](../../media/advanced-hunting-pie-chart.jpg)
*圓形圖，顯示跨越排名多個寄件者的網域之網路釣魚電子郵件的散佈*

#### <a name="file-activities-over-time"></a>一段時間的檔活動
使用`summarize`運算子搭配`bin()`函數，您可以檢查是否有一段時間的特定指示器相關的事件。 下列查詢會計算大約30分鐘內`invoice.doc`的檔案相關事件，以顯示與該檔案相關的波峰。
...
```
下方的折線圖會明確強調包含更多活動的時間`invoice.doc`週期： 

![高級搜尋查詢結果的圖像顯示為折線圖](../../media/advanced-hunting-line-chart.jpg)
折線圖，*顯示隨時間的檔相關的事件數目*
...
在您想要包含或`+`排除`-`的值上選取 [或] 按鈕，然後選取 [**執行查詢**]，以精煉您的查詢。
