...
您可以使用 Microsoft 365 商務版 Premium，在 Windows 10 電腦上自動安裝32位 Office 應用程式，並將更新維持在最新狀態。
...
若要判斷您是否有 Click-to-Run 版本的 office，請在任何 Office 應用程式中，移至 [Outlook) 中的 [ **檔** \> **帳戶** ] ( **Office 帳戶** ]。 如果您看到如下圖所示的 **Office 更新** ，則是使用 Click-to-Run 執行安裝。 
...
- **沒有64位** 的 Office app (範例： Word、Excel PowerPoint) 。 如果需要64位的 Office app，這項功能不是很好的，因為不支援從 Microsoft 365 for business 管理主控台觸發64位的 2016 Click-to-Run 版本的 Office。 
...
|現有 Click-to-Run 32 位版本的 Office，以及 Click-to-Run 32 位或64位獨立 Office app (例如 Visio、Project)   <br/> |無  <br/> |獨立應用程式不會受到影響。 套件已升級到隨選即用 32 位元版本的 Office 2016  <br/> |
 ...
 **(\*) 附注：** 由於已知的錯誤，不會升級至 Click-to-Run 32 位版本的 Office 2016。 修正程式正在進行中。 
