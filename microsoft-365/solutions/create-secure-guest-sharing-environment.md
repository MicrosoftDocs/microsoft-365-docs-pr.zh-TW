...
多重要素驗證可大幅降低帳戶遭入侵的機會。 由於來賓可能使用不符合任何控管原則或最佳做法的個人電子郵件帳戶，因此要求來賓進行多重要數驗證特別重要。 若來賓的使用者名稱和密碼遭竊，要求第二個驗證要素能大幅降低未知人員存取網站和檔案的機會。
...
透過 Azure AD 中的存取權檢閱，您可以自動定期檢閱各小組和群組的使用者存取權。 透過特別要求來賓的存取權檢閱，您可以協助確保來賓不會在非必要時持續存取貴組織的敏感性資訊。
...
2. 按一下 **[新增存取權檢閱]**。
3. 選擇 **Teams + 群組** 選項。
4. 選擇 **來賓使用者的所有 Microsoft 365 群組** 選項。 如果您想要排除任何群組，請按一下 **[選取排除群組]**。
5. 選擇 **[僅來賓使用者]** 選項，然後按一下 **[下一步 : 檢閱]**。
6. 在 **[選取檢閱者]** 下，選擇 **[群組擁有者]**。
7. 按一下 **[選取遞補檢閱者]**，選擇誰是遞補檢閱者，然後按一下 **[選取]**。
8. 在 **[指定檢閱週期]** 下，選擇 **[季度]**。
9. 選取開始日期和期間。
10. 針對 **[結束]**，選擇 **[永不]**，然後按一下 **[下一步 : 設定]**。

    ![Azure AD 存取權檢閱索引標籤的螢幕擷取畫面](../media/azure-ad-create-access-review.png)

11. 在 **[設定]** 索引標籤上，檢閱符合您商務規則的設定。
...
請注意，來賓可取得小組或群組，或個別檔案和資料夾的存取權。 取得檔案和資料夾存取權時，系統可能不會將來賓新增至任何特定群組。 如果您要對不屬於某小組或群組的來賓執行存取權檢閱，您可以在 Azure AD 中建立動態群組以包含所有來賓，然後建立該群組的存取權檢閱。 網站擁有者也可以管理[網站的來賓到期日](https://support.microsoft.com/office/25bee24f-42ad-4ee8-8402-4186eed74dea)
...
## <a name="configure-a-session-timeout-for-guests"></a>為來賓設定工作階段逾時。

若來賓的裝置不安全，要求來賓定期驗證可降低未知的使用者存取貴組織內容的可能性。 您可以在 Azure AD 中設定來賓的工作階段逾時條件式存取原則。
