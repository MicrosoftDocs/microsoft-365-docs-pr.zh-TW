...
隨著 Microsoft 365 群組和 Microsoft Teams 的使用量增加，管理員和使用者需要一種方式來清除未使用的群組和團隊。 Microsoft 365 群組到期原則可協助從系統中移除非使用中的群組，並使事情更整潔。

當群組到期時，其相關聯的所有服務 (信箱、Planner、SharePoint 網站、小組等 ) 也會被刪除。

當群組到期時，它會「虛刪除」，表示它仍可復原30天。

管理員可以指定到期期限及任何非作用中的群組，只要到達該期間結束，也不會被更新，將會被刪除。  (這包括已封存的團隊。 ) 到期期間會在建立群組時或在最後一次更新的日期時開始。 在到期之前，系統會自動將電子郵件傳送給群組擁有者，以允許使用者在其他到期間隔內更新群組。 Teams 使用者會在 Teams 中看到持續通知。

主動使用中的群組會自動更新。 下列任何動作都會自動更新群組：
- SharePoint-查看、編輯、下載、移動、共用或上傳檔案。  (查看 SharePoint 頁面不會算作自動更新的動作。 ) 
- 從群組中 Outlook 加入群組、讀取或寫入群組郵件，與 (Outlook 網頁) 上的郵件。
- Teams-來訪小組頻道。

請注意，唯一會觸發自動群組更新的 Yammer 活動，就是將檔上傳至群組內 SharePoint。
...
> [!NOTE]
> 設定和使用 Microsoft 365 群組的到期原則時，需要您擁有但不一定要為已套用到期原則之所有群組的成員指派 Azure AD 進階版授權。 如需詳細資訊，請參閱[Azure Active Directory 進階版快速](/azure/active-directory/active-directory-get-started-premium)入門。

## <a name="who-can-configure-and-use-the-microsoft-365-groups-expiration-policy"></a>神秘可以設定及使用 Microsoft 365 群組到期原則？

|角色|可以執行的動作|
|---------|---------|
|在 Azure 中 Office 365 全域系統管理員 (公司管理員) ，使用者管理員|建立、讀取、更新或刪除 Microsoft 365 群組到期原則設定。|
...
如以上所述，到期會預設為關閉狀態。 管理員必須啟用到期原則，並將其設定為生效。 若要啟用此功能，請移至 **Azure Active Directory**  >  **群組**  >  **到期** 日。 您可以在這裡設定預設的組生命週期，並指定您要提前多久的時間，將第一和第二個到期通知移至群組擁有者。
...
群組擁有者只會透過電子郵件通知。 如果群組是透過 Planner、SharePoint 或任何其他應用程式建立，到期通知永遠都透過電子郵件傳送。 如果群組是透過 Teams 建立，群組擁有者會收到透過「活動」區段進行更新的通知。 如果您的群組擁有者沒有有效的電子郵件地址，建議您在群組上啟用到期。
