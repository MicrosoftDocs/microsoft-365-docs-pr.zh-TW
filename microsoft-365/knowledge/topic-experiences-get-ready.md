...
2. [將資訊連線至 Microsoft Graph](#2-connect-information-to-microsoft-graph)
    ...
    - 針對無法移動的內容，請考慮使用 Graph 連接器來增強搜尋並準備好未來包含。
...
4. [適當保護內容](#4-secure-content-appropriately)
    ...
    - 在使用者和內容上檢查您的 Microsoft Graph 設定：
...
- [合作夥伴移轉工具和服務](https://www.microsoft.com/solution-providers)

進行最大的遷移：

- 遷移至現代網站-包括 Microsoft Teams。 雖然索引可以在任何 SharePoint 網站 (傳統或新式) 上發生，但是透過重點項目和卡片向使用者顯示主題只會發生在新式頁面上。
- 維護使用者-大部分的遷移工具可讓您在整個遷移期間對應使用者身分識別，這樣就會在遷移之後保留以所建立的屬性或修改者等屬性。 這對主題來說很重要，因為檔案的 authorship 是用來識別新增至主題頁面或卡片的專家。 
- 將服務帳戶名稱設定為描述性-在某些情況下，不可能維護所有的使用者名。 例如，如果您要遷移的是某人不再是組織員工所建立的內容。 在此執行個體中，大部分移轉工具會移動檔案，就像檔案是由系統管理員帳戶或服務帳戶所建立一樣。 如果經常發生這種情況，則該服務帳戶會以專家的身分列在主題中。 在這個情況下，該帳戶的命名會變得非常重要。 如果您使其具有描述性，您的使用者使用主題時，這些非人員帳戶的狀態將是可理解的。

## <a name="2-connect-information-to-microsoft-graph"></a>2. 連線資訊給 Microsoft Graph
...
如果您有許多傳統網站，請將設定檔頁面列為高優先順序，以轉換為新式。
...
- 鼓勵網站擁有者檢閱共用和權限。
  ...
  網站擁有者也可以前往「**網站許可權**」和「**高級許可權」設定** 頁面，查看誰具有網站許可權。

  1. 在您的網站上，選擇 [**設定**  >  **網站許可權**]。 查看哪些人列在網站擁有者、網站成員和網站訪客下。 檢查任何來賓使用者。
  2. 在 **[權限]** 頁面中，選擇 **[進階權限設定]**。 您可以檢查唯一權限，並查看誰對網站中任何項目的存取受到限制。

- 稽核 Microsoft 365 群組和 Teams，確定已適當設定為公開或私人群組或小組。 根據預設，新的 Teams 和 Microsoft 365 群組會設為私密，但初次發行時預設是公開的。 如果您已將這些技術的先前結合，您可能想要複習。 此外，小組的功能通常會在其生命週期中演進，且可能需要更新設定，以反映小組目前的使用方式。
- 請查看「所有人」、「外部使用者以外的所有人」或廣泛的安全性群組的使用。 內容可能不正確地與這些值共用。 若要查看這些群組的使用，您可以：
  - 建立沒有群組成員資格的帳戶
  - 透過此帳戶使用搜尋來探索廣泛共用的內容。
...
管理主題包括三個重要角色，包括兩個新 Azure Active Directory (AAD) 角色：知識管理員和知識管理員：
...
- 將主題植入主題清單是一種很好的作法。 主題的品質與數量是以您的內容為基礎，只要其包含在範圍中的內容，它就會建立為主題。 如果主題有足夠的資訊與證據，它就會透過 AI 建立。 植入主題是知識管理員和主題專家可協助的地方。 將人類知識與 AI 結合是品質主題的最佳路徑。 因此，如果有您預期的主題，您可以在主題中心手動建立這些主題。 這樣做會讓 AI 成為該主題相關性的強烈信號，而且會識別要與該主題產生關聯的資源和人員。
