...
使用模型標題取得或更新 SharePoint Syntex 文件瞭解模型的資訊。 (查看[範例](rest-getbytitle-method.md#examples))。
...
### <a name="get-information-about-the-contoso-contract-model"></a>取得 Contoso 合約模型的資訊

在這個範例中，Syntex 文件了解模型的名稱為 `Contoso Contract`。
...
### <a name="get-and-delete-the-contoso-contract-model-by-name"></a>根據名稱取得及刪除 Contoso 合約模型

在此範例中，Contoso 合約文件了解模型的名稱為 `Contoso Contract`。
