...
敏感度等級讓您可以在您的模型識別的文件中套用加密、共用及條件式存取原則。 例如，您希望您的模型不僅識別將上傳到您文檔庫且包含銀行帳號或信用卡號的任何財務文件，而且還對它們套用 *加密* 敏感度標籤，以限制誰可以存取該內容以及可以如何使用它。 SharePoint Syntex 模型會採用[標籤順序](../compliance/apply-sensitivity-label-automatically.md#how-multiple-conditions-are-evaluated-when-they-apply-to-more-than-one-label)規則，且不會覆寫使用者對檔案手動套用的現有標籤。 
