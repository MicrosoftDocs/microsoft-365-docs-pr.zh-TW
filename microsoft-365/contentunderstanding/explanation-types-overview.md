...
說明可用來協助您定義要在 Microsoft SharePoint Syntex 中於您的文件瞭解模型中標記及摘錄的資訊。建立說明時，您必須選取說明類型。本文協助您了解不同的說明類型及其使用方式。 

   ![說明類型](../media/content-understanding/explanation-types.png) 
   
以下為可用的說明類型：

- **片語清單**：您可以在要摘錄的文件或資訊中使用的單字、片語、數字或其他字元的清單。例如，文字字串 **轉診醫生** 在所有您要識別的「醫療轉診」文件中。</br>

- **模式清單**：列出可用來識別您要摘錄的資訊之數字、字母或其他字元的模式。例如，您可以從所有要識別的「醫療轉診」文件中，摘錄轉診醫生的 **電話號碼**。</br>

- **鄰近**：描述說明彼此之間的相近程度。例如，*街道號碼* 模式清單在 *街道名稱* 片語清單之前，而中間沒有權杖 (您將在本文後面了解權杖)。使用鄰近類型時，要求在您的模型中至少有兩個說明，否則將停用該選項。 
 
## <a name="phrase-list"></a>片語清單

片語清單說明類型通常是用來透過您的模型來識別和分類文件。如 *轉診醫生* 標籤範例中所述，它是在您要識別的文件中一致的單字、片語、數字或字元之字串。

雖然這不是一項要求，但如果您要擷取的片語位於文件中的一致位置，則可以經由說明來達到更好的成功。例如，*轉診醫生* 標籤可能會一致地位於文件的第一個段落。
...
建立可識別及摘錄文件中資訊的說明時，模式清單類型特別實用。它通常會以不同的格式呈現，例如日期、電話號碼和信用卡號碼。例如，日期可以以許多不同的格式顯示 (1/1/2020、1-1-2020、01/01/20、01/01/2020、Jan 1,2020 等)。定義模式清單可讓您的說明更有效率，方法是擷取您想要識別及摘錄資料中的任何可能變化。 

針對 **電話號碼** 範例，則會從模型識別的所有「醫療轉診」文件中，摘錄每位轉診醫生的電話號碼。建立說明時，請選取 [模式清單] 類型，以允許您預期傳回的不同格式。
...
例如，如果您建立 **日期** 模式清單，而您想要確認例如 *Jan 1, 2020* 的日期格式可被識別，您必須：
...
鄰近說明類型可協助您的模型識別資料，方法是透過定義另一個資料片段與其相似程度。例如，在您的模型中，您定義了兩個說明，分別標記客戶的 *街道地址號碼* 和 *電話號碼*。 

請注意，客戶的電話號碼永遠都出現在街道地址號碼的前面。 
...
|--|--|--|
|`Dog`|1|沒有標點符號或空格的單字。|
|`RMT33W`|1|訂位紀錄代號。其中可能含有數字和字母，但沒有標點符號。|
