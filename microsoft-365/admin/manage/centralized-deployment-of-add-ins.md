...
Microsoft Exchange 會將增益集資訊清單儲存在貴組織的租用戶中。部署增益集的系統管理員和收到這些增益集的使用者，必須使用支援 OAuth 驗證的同一版 Exchange Server。根據預設，Exchange 多租用戶和專用 VNext 部署支援 OAuth。Exchange 專用舊版和混合式內部部署的部署可設定為支援 OAuth，不過這並非預設設定。
