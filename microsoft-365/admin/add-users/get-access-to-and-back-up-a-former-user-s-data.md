...
當員工離開您的組織時，您可能想要存取其資料 (檔和電子) 郵件，並將其審閱、備份，或將其提供給新員工。
...
8. 選取 **[流覽]** ，以選取要將 Outlook 資料檔案儲存在何處 ( .pst) 。 輸入檔案名  *，然後* 選取 **[確定]** 繼續。 
    
    > [!NOTE]
    > 如果您以前已使用 export，就會出現上一個資料夾位置和檔案名。 請輸入  *其他*  的檔案名，然後選取 **[確定]**。 
  
9. 若要匯出至現有的 Outlook 資料檔案 ( .pst) 中，在 [ **選項**] 下，指定匯出檔案中已存在的專案時要執行的動作。
    
10. Select **Finish**.
    
除非已建立新的 Outlook 資料)  ( 檔或使用密碼保護的檔案，否則 Outlook 會立即開始匯出。
  
   - 如果您正在建立 Outlook 資料檔案 ( .pst) 中，選用密碼可協助保護檔案。 當 [**建立 Outlook 資料檔案**] 對話方塊出現時，在 [**密碼**] 和 [**確認密碼**] 方塊中輸入 *密碼*，然後選取 **[確定]**。 在 [ **Outlook 資料檔案密碼** ] 對話方塊中，輸入  *密碼*，然後選取 **[確定]**。
    
  - 如果您要匯出至現有的 Outlook 資料檔案 ( .pst) 受密碼保護，請在 [ **Outlook 資料檔案密碼** ] 對話方塊中輸入  *密碼*，然後選取 **[確定]**。
...
> [!TIP]
> 如果您只想匯入或還原 Outlook 資料檔案 ( .pst) 中的幾個專案，您可以開啟 Outlook 資料檔案。 然後，在功能窗格中，將 [Outlook 資料檔案資料夾] 中的專案拖曳到現有的 Outlook 資料夾中。 
