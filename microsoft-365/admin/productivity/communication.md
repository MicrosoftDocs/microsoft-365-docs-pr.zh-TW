...
當您的組織如何使用 Microsoft 365 的技術經驗和技術經驗時，生產力分數可支援與數位轉換的旅程。 貴組織的分數反映人員和技術經驗度量，而且可以與類似于您自己的組織規模的基準進行比較。 通訊類別是「人員經驗」度量的一部分。 若要深入瞭解，請參閱 [生產力評分概述](productivity-score.md) ，並閱讀 [Microsoft 的隱私權聲明](https://privacy.microsoft.com/privacystatement)。
...
為了進行通訊，我們提供主要的洞察力，其中包含與組織中的通訊相關的重要計量，結合了使用這些度量單位來計算組織得分的計分架構。
...
1. **標頭：** 提供使用一種以上通訊模式之人員的重要衡量方式，亦即
      - 透過 Exchange 傳送電子郵件
      - 透過小組傳送郵件
      - 在 Yammer 中發佈社區
...
#### <a name="trend-visualization-of-primary-insight"></a>主要洞察力的趨勢視覺化
 ...
 此圖提供主要洞察力中主要洞察力之主要衡量的趨勢，也就是您的組織中，使用一種以上的通訊模式超過過去180天的趨勢。 在這裡，每日值是在過去28天內使用多種通訊形式之人員人數的集合。

:::image type="content" source="../../media/trend-communication.png" alt-text="此圖表顯示使用一種以上的模式與時間進行通訊的使用者人數。":::

### <a name="scoring-model"></a>計分模型

組織的通訊分數是在合計 (組織) 層級衡量，不論人員是透過電子郵件、聊天和群組文章中的多個模式進行通訊，而非28天的時段。  

在個別使用者層級不提供分數。

## <a name="explore-how-your-organization-communicates"></a>探索您的組織如何進行通訊

我們也會提供資訊，協助您深入瞭解您的組織如何進行通訊。 這些額外的衡量值不會直接影響您的生產力分數，但可協助您建立行動計畫做為數位轉換的一部分。
...
3. **視覺效果：** 根據新的電子郵件執行緒是否已 @mentions，打破其回應率。
      - **使用提及的電子郵件回應 \@ ：** 彩色部分和分數代表包含 @mentions 的新電子郵件執行緒的回應率。 這個分數的構造來源如下：
          - **分子：** 新的電子郵件執行緒，包含在過去28天內已開始和接收回應的 @mentions。
