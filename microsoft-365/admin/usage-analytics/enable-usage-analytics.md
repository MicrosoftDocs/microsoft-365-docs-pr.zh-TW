...
這會啟動資料收集程式，並且會在2到48小時內完成，視租使用者的大小而定。 [ **移至 POWER BI** ] 按鈕將會啟用 () 資料收集完成時不再是灰色的。 
...
6.  按一下 [連線 **]** 之後，在 [連線 **至 Microsoft 365 流量分析]** 畫面上，輸入 [租使用者識別碼] (不含短劃線) 您已複製步驟 (1) ，然後選取 **[下一步]**。
...
所有報告都提供租使用者層級匯總。 **使用者層級的詳細資料只會在 [行事曆] 月的第一天或第15天之後變為使用**。 這會影響所有「使用者活動」下的報告 (請參閱 [流覽並利用 Microsoft 365 使用方式分析中的報告](navigate-and-utilize-reports.md) ，以取得如何查看和使用這些報告的秘訣) 。
