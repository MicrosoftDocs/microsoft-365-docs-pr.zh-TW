...
產品  |資訊
--- | ---
Windows Hello 企業版 | 您應為商務用 Windows Hello，以 Microsoft 受管理的電腦裝置的強雙因素驗證來取代密碼。 如需詳細資訊，請參閱[商務用 Windows Hello](/windows/security/identity-protection/hello-for-business/hello-identity-verification)。
...
Microsoft Defender 防毒軟體行為型、啟發式和即時防防毒保護 | Always on 掃描檔和處理威脅（可能偵測為惡意程式碼）。 如需詳細資訊，請參閱[Microsoft Defender 防毒軟體行為型、啟發式和即時防防毒保護](../../security/defender-endpoint/microsoft-defender-antivirus-in-windows-10.md)。
...
Windows Defender具有高級安全性的防火牆 | 以主機為基礎的雙向網路流量篩選裝置，Windows Defender 防火牆會封鎖進出本機裝置的未授權網路流量。 如需詳細資訊，請參閱[Windows Defender 具有高級安全性的防火牆](/windows/security/threat-protection/windows-firewall/windows-firewall-with-advanced-security)。
