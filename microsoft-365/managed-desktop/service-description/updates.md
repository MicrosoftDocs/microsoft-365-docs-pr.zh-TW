...
由 Microsoft 發行的更新是累加的並可能會歸類為品質] 或 [功能更新。
...
- **第一個**： 包含早期軟體採納者和可能需支付搶鮮版更新的裝置。 如果有程式以測試環狀測試期間未深入涵蓋的案例，此群組中的裝置可能會遇到中斷。
...
- 部署期間，Microsoft 受管理的電腦會監視的失敗或中斷 （根據診斷資料訊號和使用者支援系統）。 如果任何偵測到，會立即暫停部署，以目前和未來的所有群組。
    ...
    - 相容性問題可能會由 Microsoft 受管理的桌上型電腦 IT 系統管理員入口網站中歸檔票證報告。
...
Microsoft 受管理的電腦不支援 Windows 測試人員計畫的一部分的裝置。 Windows 測試人員計畫用來驗證搶鮮版 Windows 軟體，僅適用於非關鍵性重要裝置。 雖然這是很重要的 Microsoft 計劃，但它不是實際執行環境中的廣泛部署。 

與 Windows 測試人員組建找到任何裝置將會放入測試群組，而且無法供更新服務等級協定 (Sla)。

## <a name="bandwidth-management"></a>頻寬管理

傳遞最佳化為用於所有作業系統的系統和驅動程式更新。 它將最小化的搜尋從公司網路內的對等的更新從 Windows 更新 (WU) 服務的下載大小。
