...
ms.locfileid: "46688394"
---
# <a name="managing-office-365-endpoints"></a>管理 Office 365 端點

擁有多個辦公室位置和相連 WAN 的多數企業組織，都需要針對 Office 365 網路連線的設定。 您可以讓所有信任的 Office 365 網路要求直接通過防火牆，藉此略過所有額外的封包層級檢查或處理，進而將網路效能最佳化。 這會降低延遲和您的周邊容量需求。 要為使用者提供最佳效能的第一步，就是要找出 Office 365 網路流量。 如需有關 Office 365 網路連線的詳細資訊，請參閱 [Office 365 網路連線原則](microsoft-365-network-connectivity-principles.md)。

Microsoft 建議您存取 Office 365 網路端點，並使用 [Office 365 IP 位址和 URL Web 服務](microsoft-365-ip-web-service.md)進行變更。
...
Microsoft 正與 SDWAN 提供者合作，以啟用自動化的設定。 如需詳細資訊，請參閱 [Office 365 網路合作夥伴計畫](microsoft-365-networking-partner-program.md)。
...
Microsoft 不建議且不支援基於間接 Office 365 FQDN 的硬式編碼設定或白名單，此做法已知會導致客戶連線問題。 會封鎖 CNAME 重新導向或以其他方式不正確地解析 Office 365 DNS 項目的 DNS 解決方案，可以透過 DNS 條件式轉送 (限定於直接使用的 Office 365 FQDN) 搭配啟用 DNS 遞迴來解決。 許多協力廠商網路周邊產品會使用 [Office 365 IP 位址和 URL Web 服務](microsoft-365-ip-web-service.md)，在其設定中原生整合建議的 Office 365 端點白名單。
