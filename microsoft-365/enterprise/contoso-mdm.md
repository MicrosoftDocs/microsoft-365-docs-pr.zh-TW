...
適用于 enterprise 的 Microsoft 365 包括 Intune 和一組支援行動裝置和應用程式管理及安全性的 Azure 服務。

Contoso 有許多啟用行動的員工。 有些地方的辦事處位於 Contoso 位置，有些則沒有辦事處。 Contoso 需要一種方式來啟用員工生產力，但保留裝置、儲存在這些裝置上的 Contoso 資料，以及應用程式行為的安全性。

## <a name="plan"></a>計劃

Contoso 已針對企業版的 Microsoft 365 識別行動裝置管理的下列 Intune 使用案例：

- 保護 Exchange Online 電子郵件和資料，讓行動裝置可以安全地存取。
...
- 保護內部部署電子郵件和資料，讓行動裝置可以安全地存取它，因為沒有內部部署的 Microsoft Exchange 伺服器。
...
- 針對註冊及 Intune 設定和裝置型條件式存取原則的裝置建立 Azure Active Directory (Azure AD) 群組
...
- 對於未註冊的裝置，請將一組行動應用程式管理 (MAM) 原則，以要求驗證存取 Microsoft 365 服務
- 建立 Intune 原則以強制執行：
  - 允許的應用程式。
  - 裝置加密，以協助防止未經授權的存取。
  - 六位數的 PIN 碼或密碼。
  - 非使用中的超時期限。
  - 防病毒和惡意程式碼保護，以及 Windows Defender Windows 10 裝置上的簽名更新。
  - 在包含最新安全性更新的 Windows 10 裝置上自動更新。
  - 將憑證推入受管理的裝置。
...
[Microsoft 365 的裝置管理](device-management-roadmap-microsoft-365.md)
