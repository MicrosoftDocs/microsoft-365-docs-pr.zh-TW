...
如果您是較大的組織，且使用的是 Microsoft 365 混合式身分識別模型，您會有更多驗證選項。 如果您已將安全性基礎結構放在適當的次要驗證方法中，請針對適當的驗證方法， [設定 MFA 並設定](../admin/security-and-compliance/set-up-multi-factor-authentication.md) 每一個專用全域管理員帳戶。
...
如需詳細資訊，請參閱 [Microsoft 365 的 MFA](../admin/security-and-compliance/multi-factor-authentication-microsoft-365.md)。
...
如需詳細資訊，請參閱 [AZURE AD 特權身分識別管理](/azure/active-directory/active-directory-privileged-identity-management-configure)。
...
在伺服器上執行的 SIEM 軟體會即時分析應用程式和網路硬體所建立的安全性警示和事件。 若要讓您的 SIEM 伺服器在其分析和報告功能中包含 Microsoft 365 的安全性警示和事件，請整合 Azure AD 至您的 SEIM。 請參閱 [Azure 記錄整合簡介](/azure/security/security-azure-log-integration-overview)。
