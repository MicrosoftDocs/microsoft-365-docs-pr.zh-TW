...
本文說明保護雲端服務存取權的常見建議原則，包括使用 Azure Active Directory (Azure AD) 應用程式 Proxy 發佈的內部部署應用程式。 
...
|雲端應用程式或動作|Include| **選取應用程式**：選取您想要套用此規則的應用程式。 例如，選取 [Exchange Online]。||
|條件| | |設定您的環境和需求特有的條件。|
||登入風險||請參閱下表中的指南。|
|||||
