...
如果您略過此選項，使用者會看到一般的登入畫面，並且可能無法確定其登入的是貴組織的網站。
...
隨著將目錄同步處理，使用者可以使用他們用來登入電腦和存取內部部署資源所用的相同認證來登入 Microsoft 365 和其他 Microsoft 雲端服務。
...
使用您的使用者帳戶名稱和 Azure Multi-Factor Authentication.登入入口網站。 您應該會在登入頁面上看到您的自訂商標元素。
