...
| 步驟 (s)  | 描述 | 影響 |
|:-------|:-------|:-------|
| 從 Power BI 訂閱中移除不會從 Microsoft Cloud Deutschland Power BI 遷移至 Office 365 服務的物件。 | 遷移 Power BI 服務會要求客戶採取行動，以刪除特定的專案，例如資料集和儀表板。 | <ul><li>管理員可能必須將下列專案從其訂閱中移除： </li><li>Real-Time 資料集 (例如，流式傳送或推入資料集)  </li><li>Power BI 內部部署資料閘道設定和資料來源 </li></ul>|
||||
...
> [!NOTE]
> 您的 Microsoft Azure 服務的遷移必須在您的 Office 365 租使用者達到遷移階段3之前啟動，且必須在完成遷移階段8之前完成。
