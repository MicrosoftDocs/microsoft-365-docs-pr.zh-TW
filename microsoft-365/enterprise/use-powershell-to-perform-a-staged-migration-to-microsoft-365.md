 ...
 **設定許可權** 您用來連線至內部部署 Exchange 組織的內部部署使用者帳戶 (也稱為「遷移系統管理員) ，必須具備必要的許可權，才能存取您要遷移到 Microsoft 365 的內部部署信箱。 當您在此程式中建立遷移端點時，使用此程式來連線到電子郵件系統時，就會使用此使用者帳戶 ([步驟3：建立遷移端點](use-powershell-to-perform-a-staged-migration-to-microsoft-365.md#BK_Endpoint)) 。
