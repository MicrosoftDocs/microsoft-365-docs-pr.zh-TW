...
Microsoft SharePoint 2010 和 SharePoint Server 2010 會在 **年4月 13 2021 日** 到達支援的支援。 本文提供的資源可協助您將現有的 SharePoint 伺服器2010資料移轉至 Microsoft 365 中的 SharePoint 線上，或升級內部部署 SharePoint Server 2010 環境。
...
> [!NOTE]
> 軟體生命週期一般會從最初發行的10年持續。 [Microsoft 解決方案供應商](https://go.microsoft.com/fwlink/?linkid=841249) 可以協助您升級至下一個版本的軟體，或遷移至 Microsoft 365 遷移 (或兩者) 。 此外，請確定您已注意到重要基礎技術的支援結束日期，尤其是針對 SharePoint 所用的 Microsoft SQL Server 版本。 如需詳細資訊，請參閱 [固定生命週期原則](https://support.microsoft.com/help/14085)。
...
SharePoint Server 2013 和 SharePoint Foundation 2013 可以在您自己的伺服器上安裝內部部署。 您也可以使用線上 SharePoint，這是 Microsoft 365 的一部分線上服務。 您可以選擇：
...
> [!NOTE]
> SharePoint 系統管理員可以建立 Microsoft 365 訂閱、設定新的 SharePoint 線上網站，然後徹底從 SharePoint 伺服器2010中裁掉，只需要將基本的檔帶至全新的網站。 然後，任何剩餘的資料都可以從 SharePoint Server 2010 網站耗盡為內部部署封存。
...
如果您升級為 SharePoint Server 2013 或 SharePoint Server 2016，並決定將資料放入 SharePoint 線上，您可以使用 [SharePoint 遷移 API](https://support.office.com/article/Upload-on-premises-content-to-SharePoint-Online-using-PowerShell-cmdlets-555049c6-15ef-45a6-9a1f-a1ef673b867c?ui=en-US&amp;rs=en-US&amp;ad=US) 將資訊遷移至 OneDrive 的商務。
...
需要一些時間，並計畫追蹤從 SharePoint 2010 到 SharePoint Server 2016 的整個路徑。 升級涉及硬體的成本 (SQL server 也必須升級) 、軟體和管理。 此外，自訂專案可能也需要升級或甚至放棄。 在升級 SharePoint 伺服器陣列之前，請務必先記錄重要自訂專案。
...
請先比較硬體和軟體需求。 如果您目前的環境不符合基本需求，您可能必須先升級伺服器陣列或 SQL server 中的硬體。 
...
請務必熟悉現有類型的混合式，以及如何設定內部部署 SharePoint 伺服器陣列與 Microsoft 365 訂閱之間的連線。
...
如果您決定建立新的 Microsoft 365 網站，並視需要手動將資料移轉至它，請檢查您的 [Microsoft 365 選項](https://www.microsoft.com/microsoft-365/)。
...
若要從 SharePoint 2007 到 SharePoint 伺服器2016的完整途徑，則表示有大量的時間需要，也就是必須升級硬體 (SQL server) 、軟體和管理成本。 根據功能的重要性，必須升級或放棄自訂專案。
...
您可能想要建立 Microsoft 365 開發/測試環境，您可以使用 [測試實驗室指南](m365-enterprise-test-lab-guides.md)加以設定。 在您取得試用版或購買的 Microsoft 365 訂閱後，您可以在 SharePoint Online 中建立網站集合、網站及文件庫，以供您遷移資料。 您可以使用遷移 API 手動遷移，或者，如果您想要透過混合式嚮導將「我的網站」內容遷移至 OneDrive 供商務使用。

> [!NOTE]
> 若要使用 [混合] 選項，您的 SharePoint 伺服器2010伺服器陣列必須先升級內部部署，以 SharePoint 伺服器2013或2016。 SharePoint Foundation 2010 和 SharePoint Foundation 2013 不支援線上 SharePoint 的混合式連線。
...
此海報說明您可以採取的各種途徑，以避免使用 Office 2010 用戶端和伺服器產品及 Windows 7 的支援，並會反白顯示 Microsoft 365 企業版中的慣用路徑和選項支援。
