...
ms.locfileid: "50916665"
---
# <a name="managing-expressroute-for-office-365-connectivity"></a>管理 ExpressRoute for Office 365 連線

ExpressRoute Office 365 提供其他路由路徑，以到達許多 Office 365 服務，而不需要將所有流量送出至網際網路。 雖然仍然需要網際網路 Office 365 連線，但 Microsoft 透過 BGP 透過您的網路廣播的特定路由，除非您的網路中有其他設定，否則請直接 ExpressRoute 電路。 您可能想要設定以管理此路由的三個常見方面包括前置詞篩選、安全性和符合性。
  
> [!NOTE]
> Microsoft 變更了如何檢查 Azure ExpressRoute 的 Microsoft 對等路由網域。 從2017年7月31日起，所有 Azure ExpressRoute 客戶都可以直接從 Azure 系統管理主控台或透過 PowerShell 啟用 Microsoft 對等功能。 啟用 Microsoft 對等專案後，任何客戶都可以建立路由篩選器，以接收 Dynamics 365 客戶參與應用程式 (以前稱為 CRM Online) 的 BGP 路由播發。 需要 Office 365 Azure ExpressRoute 的客戶必須先從 Microsoft 取得檢查，然後才能建立 Office 365 的路由篩選。 請與您的 Microsoft 帳戶小組聯繫，以瞭解如何要求複查以啟用 Office 365 ExpressRoute。 嘗試為 Office 365 建立路由篩選的未授權訂閱將會收到[錯誤訊息](https://support.microsoft.com/kb/3181709)
...
為了避免使用[Office 365 URLs 及 IP 位址範圍](./urls-and-ip-address-ranges.md)以產生首碼篩選清單，有許多原因。 包括下列專案：
  
- Office 365 的 IP 首碼會經常進行許多變更。

- Office 365 URLs 和 IP 位址範圍的設計是用來管理防火牆允許清單和 Proxy 基礎結構，而非路由傳送。

- Office 365 URLs 和 IP 位址範圍並未涵蓋 ExpressRoute 連線範圍內的其他 Microsoft 服務。
...
|篩選 Office 365 IP 範圍  <br/> [!CAUTION] Not-Recommended |**高：** 客戶會根據定義的 Office 365 IP 首碼來篩選路由。  <br/> |客戶必須執行每月更新的可靠的變更管理程式。  <br/> [!CAUTION] 此解決方案需要大量的更新。 未在時間執行的變更可能會導致服務中斷。   |

使用 Azure ExpressRoute 連接至 Office 365 是以特定 IP 子網的 BGP 通告為基礎，該網路子網代表部署 Office 365 端點的網路。 由於 Office 365 的全域性質和構成 Office 365 的服務數目，客戶常常需要在其網路上管理其所接受的廣告。 如果您關心的是在您的環境中宣告的首碼數目，則[BGP 社區](https://support.office.com/article/Using-BGP-communities-in-ExpressRoute-for-Office-365-scenarios-preview-9ac4d7d4-d9f8-40a8-8c78-2a6d7fe96099)功能可讓您將廣告篩選成一組特定的 Office 365 服務。 這項功能現在已在預覽中。
  
不論您如何管理來自 Microsoft 的 BGP 路由廣告，當您與透過網際網路電路只連接至 Office 365 進行比較時，就不會對 Office 365 服務帶來任何特殊的公開。 不管客戶用來連線至 Office 365 的電路類型為何，Microsoft 都會維持相同的安全性、相容性和效能等級。
...
當電腦連接到 Office 365 時，無論是透過網際網路還是 ExpressRoute 電路進行連線，都可以連接到同一組端點。 不論使用哪一種電路，Microsoft 建議您將 Office 365 服務視為比一般網際網路目的地更受信任。 您的外寄安全性控制措施應重點放在埠和通訊協定上，以降低曝光並最小化不斷維護。 必要的埠資訊可在[Office 365 端點](./urls-and-ip-address-ranges.md)參考文章中取得。
  
針對新增的控制項，您可以使用 proxy 基礎結構內的 FQDN 層級篩選，來限制或檢查部分或所有網路要求的網際網路或 Office 365。 維護 fqdn 清單時，隨著功能的發行和 Office 365 選項的演化，需要更強大的變更管理，並追蹤已發佈的[Office 365 端點](./urls-and-ip-address-ranges.md)的變更。
  
> [!CAUTION]
> Microsoft 建議您不要只依賴 IP 首碼來管理要 Office 365 的輸出安全性。
...
- 從 Exchange Online 租使用者傳送至內部部署主機的郵件。

- SharePoint線上郵件從 SharePoint 線上傳送至內部部署主機。
...
Microsoft 建議您透過網際網路電路接受這些連線，而不是 ExpressRoute 電路，以降低複雜性。 若您的相容性或效能需求規定必須透過 ExpressRoute 電路接受這些輸入連線，則建議使用防火牆或反向 proxy 來限定已接受的連線的範圍。 您可以使用[Office 365 端點](./urls-and-ip-address-ranges.md)來判斷正確的 fqdn 和 IP 首碼。
  
### <a name="compliance"></a>合規性

我們不依賴您用於任何符合性控制的路由路徑。 不論您是透過 ExpressRoute 或網際網路電路連接至 Office 365 服務，我們的規範控制措施都不會變更。 您應該查看 Office 365 的不同法規遵從性和安全性憑證等級，以找出符合組織需求的最佳選擇。
