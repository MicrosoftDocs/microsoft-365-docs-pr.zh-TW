...
如果您已整合內部部署 Active Directory 網域服務 (AD DS) 與 Azure Active Directory (Azure AD) 同步處理內部部署環境與 Microsoft 365，您也可以檢查同步處理的狀態。
  
## <a name="view-directory-synchronization-status"></a>檢視目錄同步處理狀態

- 登入 Microsoft 365 系統 [管理中心](https://admin.microsoft.com)，然後選擇首頁上的 [ **DirSync 狀態**]。
...
如果您的目錄同步處理有問題，錯誤也會列在此頁面上。 如需您可能會遇到之不同錯誤的詳細資訊，請參閱[在 Microsoft 365 中識別目錄同步處理錯誤](identify-directory-synchronization-errors.md)。
  
|項目|功能|
|:-----|:-----|
|**已驗證網域** | 您已驗證的 Microsoft 365 租使用者中的網域數目。 |
|**未驗證的網域** | 您已新增但未驗證的網域。 |
...
|**目錄同步處理用戶端版本** | 包含新版本的 Azure AD 連線已發行的下載連結。 |
|**目錄同步處理服務帳戶** | 顯示 Microsoft 365 目錄同步處理服務帳戶的名稱。 |
|||
