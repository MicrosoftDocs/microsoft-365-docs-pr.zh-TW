...
如果您仍在使用 Azure Active Directory (Azure AD) 同步處理 (DirSync)，請參閱[如何疑難排解 Microsoft 365 中的 Azure Active Directory 同步處理工具安裝和設定精靈錯誤訊息](/troubleshoot/azure/active-directory/installation-configuration-wizard-errors)，以取得安裝同步處理的系統需求、所需的權限及如何疑難排解常見錯誤的相關資訊。 
  
若要從 Azure AD 同步服務更新至 Azure AD Connect，請參閱 [[升級指示]](/azure/active-directory/hybrid/how-to-dirsync-upgrade-get-started)。
...
- [身分識別同步及重複屬性復原](/azure/active-directory/hybrid/how-to-connect-syncservice-duplicate-attribute-resiliency)
...
- [針對 Azure AD Connect 的連線問題進行疑難排解](/azure/active-directory/hybrid/tshoot-connect-connectivity)
- [Azure AD Connect 帳戶和權限](/azure/active-directory/hybrid/reference-connect-accounts-permissions)
- [Azure AD Connect 同步：如何管理 Azure AD 服務帳戶](/azure/active-directory/hybrid/how-to-connect-azureadaccount)
...
- [使用 Azure AD Connect 同步處理來實作密碼雜湊同步處理](/azure/active-directory/hybrid/how-to-connect-password-hash-synchronization)
...
- 您是否準備好僅管理雲端中的物件？ 或者，是否有透過內部部署刪除但卡在雲端中的物件？ 請參閱此 [在同步處理期間進行疑難排解錯誤](/azure/active-directory/hybrid/tshoot-connect-sync-errors) 和 [支援文章](/troubleshoot/azure/active-directory/cannot-manage-objects)以獲取解決這些問題的指南。
...
- 如需此問題的詳細資訊，請參閱 [[這裡]](/troubleshoot/azure/active-directory/exceed-number-objects-synced)。
   
## <a name="other-resources"></a>其他資源

- [修正使用者主要名稱重複問題的腳本](/samples/browse/?redirectedfrom=TechNet-Gallery)
    
- [如何準備無法路由傳送的網域 (例如.local 網域) 以用於目錄同步處理](prepare-a-non-routable-domain-for-directory-synchronization.md)
    
- [計算已同步處理物件之總數的腳本](/samples/browse/?redirectedfrom=TechNet-Gallery)
