...
5. 在 [**檢閱並完成新增群組**] 窗格中，輸入**GlobalAdmins**，，然後按 [**下一步**。
7. 在群組清單中，按一下 [ **GlobalAdmins**群組]。
8. 在**GlobalAdmins** ] 窗格中，按一下 [**成員**]，然後按一下 [**檢視所有及管理成員**。
9. 在**GlobalAdmins** ] 窗格中，按一下 [**新增成員**、 選取**DedicatedAdmin**帳戶與您的全域系統管理員帳戶，，然後按一下 [**儲存 > Close > 關閉**。
...
3. 在 [**條件式存取 – 原則**] 窗格中，按一下 [**基準線原則： 系統管理員 （預覽） 需要 MFA**。
4. 在 [**比較基準原則**] 窗格中，按一下 [**立即 > 儲存使用原則**。

以全域系統管理員帳戶驗證登入風險中或高時此第二個原則封鎖存取。

1. 在 [**條件式存取 – 原則**] 窗格中，按一下 [**新增原則**]。
2. 在 [**新增**] 窗格中，輸入**名稱**的**全域系統管理員**。
3. 在 [**指派**] 區段中，按一下 [**使用者和群組**。
4. 在 [**使用者和群組**] 窗格的 [**包含**] 索引標籤中，按一下 [**選取使用者及群組 > 使用者和群組 >，請選取**。
5. 在 [**選取**] 窗格中，按一下 [ **GlobalAdmins** ] 群組中，然後按一下 [**選取 > 完成**。
6. 在 [**指派**] 區段中，按一下 [**條件**]。
7. 在 [**條件**] 窗格中，按一下 [**登入風險**、 按一下 [**是]** 的**設定**，按一下 [**高**] 和 [**中等**，，然後按一下**選取**並**完成**。
8. 在 [**新增**] 窗格中的 [**存取控制**] 區段中，按一下 [**授與**]。
9. 在 [**授與**] 窗格中，按一下 [**封鎖存取**]，然後按一下 [**選取**。
10. 在 [**新增**] 窗格中，**啟用原則**，請按一下 [ **** ，然後按一下 [**建立**]。
11. 關閉 [ **Azure 入口網站**和**Microsoft 365 系統管理中心**] 索引標籤。

若要測試的第一個原則，請登出，並使用 DedicatedAdmin 帳戶登入。 應該會提示您設定 MFA。 此示範，將第一個會被套用原則。
