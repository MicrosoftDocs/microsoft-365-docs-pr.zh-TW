...
將命令貼到 [記事本]。 針對 *$dispName* 和 *$roleName* 變數，以其值取代描述文字。 移除 \< and > 字元，但保留引號。 將修改過的列貼到 [Windows PowerShell 的 Microsoft Azure Active Directory 模組] 視窗中，以執行這些行。 或者，您也可以使用 Windows PowerShell 整合型腳本環境 (ISE) 。
...
複製命令，並將其貼到 [記事本]。 用於 **$upnName** 和 **$roleName** 變數。 以其值取代描述文字。 移除 \< and > 字元，但保留引號。 將修改過的列貼到 Microsoft Azure Active Directory Module for Windows PowerShell] 視窗中執行。 或者，您也可以使用 Windows PowerShell ISE。
