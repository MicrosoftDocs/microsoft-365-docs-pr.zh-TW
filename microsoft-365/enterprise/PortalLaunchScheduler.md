...
> - 如果您的組織使用的是[Viva](/SharePoint/viva-connections)連線，使用者可能會在 Microsoft Teams 應用程式欄中看到您組織的圖示，但是當圖示已選取時，使用者將無法存取入口網站，直到其 wave 啟動為止
...
- 需要有系統管理員認證，才能使用[SharePoint PowerShell](/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell)
- 一個波形的最低需求
...
SharePoint 入口網站啟動排程器工具最初隻可透過[SharePoint](/powershell/sharepoint/sharepoint-online/introduction-sharepoint-online-management-shell)取得，PowerShell 而且會繼續透過 PowerShell 使用此方法的客戶取得支援。 本文開頭的相同附注適用于這兩個版本的入口網站啟動排程器。
