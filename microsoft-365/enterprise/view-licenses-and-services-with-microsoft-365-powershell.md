...
- **授權方案** 這些也稱為授權計畫或 Microsoft 365 計畫。 授權計畫定義使用者可用的 Microsoft 365 服務。 您的 Microsoft 365 訂閱可能包含多個授權計畫。 授權方案範例 Microsoft 365 E3。
    
- **服務** 這些也稱為服務方案。 服務是每個授權方案中可用的 Microsoft 365 產品、功能及功能，例如 Exchange Online 和 Microsoft 365 Apps 企業版 (先前命名 Office 365 專業增強版) 。 使用者可以將多個授權指派給他們，以不同授權方案授與不同服務的存取權。
    
- **授權** 每個授權方案都包含您購買的授權數目。 您可以將授權指派給使用者，讓他們可以使用授權方案所定義的 Microsoft 365 服務。 每個使用者帳戶都至少需要一個授權方案的授權，以便他們可以登入 Microsoft 365 並使用服務。
    
您可以使用 PowerShell 進行 Microsoft 365，以查看 Microsoft 365 組織中可用授權方案、授權及服務的詳細資料。 如需不同 Office 365 訂閱中可用之產品、功能和服務的詳細資訊，請參閱[Office 365 計畫選項](/office365/servicedescriptions/office-365-platform-service-description/office-365-plan-options)。
...
- **SkuPartNumber：** 顯示您的組織可用的授權方案。 例如， `ENTERPRISEPACK` 為 Office 365 企業版 E3 的授權方案名稱。
...
- **AccountSkuId：** 使用語法顯示組織可用的授權方案 `<CompanyName>:<LicensingPlan>` 。  _\<CompanyName>_ 是您在 Microsoft 365 登記時所提供的值，且對於您的組織而言是唯一的。 _\<LicensingPlan>_ 所有人的值都相同。 例如，在值 `litwareinc:ENTERPRISEPACK` 、公司名稱以及 `litwareinc` 授權方案名稱 `ENTERPRISEPACK` ，也就是 Office 365 企業版 E3 的系統名稱。
