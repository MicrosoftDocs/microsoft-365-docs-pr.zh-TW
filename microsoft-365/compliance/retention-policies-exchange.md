...
計時器工作最多可能需要 7 天才能執行，且 Exchange 位置必須包含至少 10 MB。
...
您可以使用 PowerShell 從保留原則中排除特定類型的 Exchange 項目。 例如，您可以排除語音信箱訊息、IM 交談，與信箱中的其他商務用 Skype Online 內容。 您也可以排除行事曆、筆記和工作項目。 此功能僅在使用 PowerShell 時才可供使用；當您在 Microsoft 365 合規性中心使用精靈建立保留原則時，無法使用此功能。
  
若要在保留原則中排除您選取的 Exchange 項目類型，請使用 `New-RetentionComplianceRule` 和 `Set-RetentionComplianceRule` Cmdlet 搭配 `ExcludedItemClasses` 參數。
