...
使用 Microsoft 365 規範中心內的 Globanet 連接器，將以文字分隔的資料匯入至您的 Microsoft 365 組織中的使用者信箱，並將其封存。 Globanet 提供 [文字分隔的連接器](https://globanet.com/text-delimited) ，可設定為定期從協力廠商資料來源捕獲專案 () 並將這些專案匯入至 Microsoft 365。 連接器會將內容從文字分隔的資料來源轉換為電子郵件訊息格式，然後將這些專案匯入至 Microsoft 365 中的使用者信箱。
...
4. 連接器會使用 [步驟 3] 中所述之自動使用者對應的 *電子郵件* 屬性值，將已轉換的訊息項目匯入特定使用者的信箱。 在使用者信箱中，會建立名為 **文字分隔** 之收件匣資料夾中的新子資料夾，然後將訊息項目匯入該資料夾。 連接器會使用 *Email* 屬性的值來執行此動作。 每封郵件都包含此內容，它會填入郵件每一位參與者的電子郵件地址。
