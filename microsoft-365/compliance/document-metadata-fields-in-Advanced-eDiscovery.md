...
下表列出在 Advanced eDiscovery 的情況下，審閱集中的檔的元資料欄位。 表格提供下列資訊：
...
|||Converted_file_path|轉換後的匯出檔案的路徑。 僅供 Microsoft 內部使用。|
...
|檔修訂|Doc_Version|Doc_Version|檔中繼資料中的修訂。|
|檔主題||Doc_subject|使用檔中繼資料的主旨。|
|文件範本||Doc_template|檔中繼資料中的範本。|
|DocLastSavedBy||Doc_last_saved_by|上次儲存檔的使用者名稱。|
...
|||Export_native_path|匯出檔案的路徑。|
|已解壓縮內容類型||Native_type|已解壓縮內容類型，格式為 mime 類型;例如， **image/jpeg**|
...
|InputFileExtension||Original_file_extension|檔案的原始檔案副檔名。|
|InputFileID||Input_file_ID|複查集中最上層專案的檔案識別碼。 若為附件，此識別碼將會是父項的 ID。 這可以用來組合群組。|
...
|位置|位置|位置|指出檔來源的位置類型的字串。<br /><br />匯 **入的資料**-非 Office 365 資料<br />**Teams** Microsoft Teams<br />**Exchange** Exchange 信箱<br />**SharePoint** SharePoint 網站<br />**OneDrive** OneDrive 帳戶|
|位置名稱|LocationName|Location_name|識別專案來源的字串。 若為 exchange，這將是信箱的 SMTP 位址。針對 SharePoint 和 OneDrive，為網站集合的 URL。|
|||Marked_as_pivot|此檔案是近乎重複的集合中的樞軸。|
...
|Received|Received|Email_date_received|以 UTC 接收電子郵件的日期和時間。|
...
|||Redacted_file_path|匯出中 redacted 取代檔案的路徑。|
|||Redacted_text_path|匯出中 redacted 文字檔取代的路徑。 僅供 Microsoft 內部使用。|
...
> [!NOTE]
> 如需針對 Advanced eDiscovery 案例收集資料時搜尋 Office 365 內容位置時可搜尋屬性的詳細資訊，請參閱[內容搜尋的關鍵字查詢和搜尋條件](keyword-queries-and-search-conditions.md)。
