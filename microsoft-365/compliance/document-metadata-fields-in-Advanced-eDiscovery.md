...
|日期|日期|日期|Date 是取決於檔案類型的計算欄位。<br /><br />電子郵件：傳送日期<br />電子郵件附件：檔的上次修改日期; 如果無法使用，則為父項的傳送日期<br />內嵌檔：檔的上次修改日期;如果無法使用，則為父項的上次修改日期<br />SPO 檔（包括新式附件）： SharePoint 上次修改日期;如果無法使用，檔會以上次修改日期<br />非 Office 365 檔：上次修改日期<br />會議：會議開始日期<br />VoiceMail：傳送日期<br />IM：傳送日期|
|其他路徑|Dedupedcompoundpath|Deduped_compound_path|完全重複的檔複合路徑清單（email：根據內容，檔：以雜湊為基礎）。|
|其他保管人|DedupedCustodians|Deduped_custodians|完全重複的檔的保管人清單（根據內容，電子郵件是根據內容，針對檔，以雜湊為基礎）。|
|其他檔案 IDs|DedupedFileIds|Deduped_file_IDs|完全重複的檔 IDs 清單（根據內容，電子郵件會根據內容，針對檔，以雜湊為基礎）。|
...
|EmailThread*||Email_thread|郵件在電子郵件集中的位置;會包含從根目錄到目前郵件的節點 IDs，並以句點（.）來分隔。|
...
|FamilyDuplicateSet*||Family_duplicate_set|與彼此完全重複的系列的數值識別碼（相同內容和所有相同的附件）。|
...
|修改檔案系統日期||File_system_date_modified|從檔案系統修改的日期（僅適用于非 Office 365 資料）。|
