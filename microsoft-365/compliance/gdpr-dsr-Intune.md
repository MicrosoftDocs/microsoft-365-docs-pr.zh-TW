...
<span id="_Delete" class="anchor"></span>資料主體可能會要求您限制對其個人資料的處理。我們同時提供了 Azure 入口網站，以及既有的應用程式開發介面 (API) 或使用者介面 (UI)。這些體驗讓企業客戶的租用戶系統管理員能透過資料匯出和資料刪除的組合，來管理這樣的 DSR。如需詳細資訊，請參閱[處理個人資料](https://docs.microsoft.com/intune/privacy-data-store-process#processing-personal-data)。
