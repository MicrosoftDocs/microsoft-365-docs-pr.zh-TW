...
- 所有專案 (包括內容和中繼資料) 都會重新編制索引，這樣就能在複查案例資料時完全搜尋評審集中的所有資料。 當您在案例調查期間搜尋考核集中的資料時，重新編制資料的索引會產生徹底且快速的搜尋。
...
當您將搜尋內容新增至現有的評審集或建立新的內容時，您可以選擇如何收集要檢查的內容：

- **會話複查集** -新增至審閱集的專案將會針對串接的對話啟用，以協助查看前後交談內容中的內容。請參閱本文中的詳細資訊 [交談回顧集合]

- **啟用新式附件的檢索** -使用此控制項包含集合中新式附件或連結的檔案，以便進一步複查;進一步瞭解可供群組此內容的新可搜尋功能變數名稱，請參閱 [advanced ediscovery 中的檔元資料欄位]

- **包括版本 SharePoint (Beta) ** -此控制項可讓您根據集合的版本限制和搜尋參數收集 SharePoint 檔案的所有版本。注意：此控制項會大幅增加集合的大小

將資料新增至審閱集是一個長時間執行的程式。 此套裝程式括從 Microsoft 365 (中的原始資料來源收集項目，例如，從信箱和網站) ，將其複製到 Azure 存放位置 (此複製程式也稱為 *攝取*) ，然後重新編制專案的索引。 您可以在 [ **作業** ] 索引標籤或 [ **搜尋** ] 索引標籤上，透過監視 [ **新增資料至審閱集合** ] 欄中的狀態，追蹤進度。 完成審閱集處理之後，按一下案例中的 [ **檢查集合** ] 索引標籤，然後按一下 [複查集]，以開始篩選、檢查、標示及匯出審閱集中的資料。
