...
將資料新增至審閱集是一個長時間執行的程式。 此套裝程式括從 Microsoft 365 （例如，從信箱和網站）收集原始資料來源中的專案，將其複製到 Azure 存放區位置（此複製程式也稱為*攝取*），然後重新編制專案的索引。 您可以在 [**作業**] 索引標籤或 [**搜尋**] 索引標籤上，透過監視 [**新增資料至審閱集合**] 欄中的狀態，追蹤進度。 完成審閱集處理之後，按一下案例中的 [**檢查集合**] 索引標籤，然後按一下 [複查集]，以開始篩選、檢查、標示及匯出審閱集中的資料。
