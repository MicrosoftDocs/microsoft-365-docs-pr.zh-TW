...
In-Place 保留、訴訟暫止及[Microsoft 365 保留原則](https://go.microsoft.com/fwlink/?LinkID=827811)（在安全性 & 規範中心建立）可讓您保留信箱內容，以符合法規遵從性和 eDiscovery 需求。 預設會在郵件的 [收件者] 和 [副本] 欄位中包含收件者的相關資訊，但您的組織可能需要能夠搜尋並再現郵件的所有收件者的詳細資料。 這包括： 
...
Exchange Online 和 Exchange Server 2013 （累計更新7和更新版本）會保留密送和展開的通訊群組收件者的相關資訊。 您可以使用 Exchange 系統管理中心（EAC）中的 In-Place eDiscovery 搜尋或安全性 & 規範中心的內容搜尋來搜尋此資訊。 
...
案例1： John 是美國銷售通訊群組的成員。 此表格顯示在王俊元透過通訊群組直接或間接將郵件傳送給 John 時的 eDiscovery 搜尋結果。
  
|**當您搜尋王俊元的信箱以傳送郵件時 .。。**|**而且會傳送郵件時使用 .。。**|**結果包含郵件？**|
|:-----|:-----|:-----|
|To： John  <br/> |John on TO  <br/> |是  <br/> |
|To： John  <br/> |美國銷售人員  <br/> |是  <br/> |
|若要：美國銷售  <br/> |美國銷售人員  <br/> |是  <br/> |
|抄送： John  <br/> |位於 CC 的 John  <br/> |是  <br/> |
|抄送： John  <br/> |US-副本上的銷售  <br/> |是  <br/> |
|抄送：美國銷售  <br/> |US-副本上的銷售  <br/> |是  <br/> |
   
案例2：王俊元會將電子郵件傳送至 John （收件者/Cc）和插孔（直接或透過通訊群組以間接方式）。 下表顯示 eDiscovery 搜尋結果。
  
|**當您搜尋 .。。**|**已傳送的郵件 .。。**|**結果包含郵件？**|**附註**|
|:-----|:-----|:-----|:-----|
|王俊元的信箱  <br/> |To/Cc： John  <br/> |是  <br/> |會顯示指示是否已 Bcc'ed 的插座。  <br/> |
|王俊元的信箱  <br/> |密送：插座  <br/> |是  <br/> |會顯示指示是否已 Bcc'ed 的插座。  <br/> |
|王俊元的信箱  <br/> |密送：插座（透過通訊群組）  <br/> |是  <br/> |在電子檔探索搜尋預覽、匯出及記錄檔中，Bcc'ed 通訊群組的成員清單（已展開郵件時即會顯示）。  <br/> |
|John 的信箱  <br/> |To/Cc： John  <br/> |是  <br/> |沒有任何指示的密件副本收件者。  <br/> |
|John 的信箱  <br/> |密送：插孔（直接或透過通訊群組）  <br/> |否  <br/> |密送資訊不會儲存在傳送給收件者的郵件中。 您必須搜尋寄件者的信箱。  <br/> |
|插座的信箱  <br/> |收件者/Cc： John （直接或透過通訊群組）  <br/> |是  <br/> |To/Cc 資訊會包含在傳遞給所有收件者的郵件中。  <br/> |
|插座的信箱  <br/> |密送：插孔（直接或透過通訊群組）  <br/> |否  <br/> |密送資訊不會儲存在傳送給收件者的郵件中。 您必須搜尋寄件者的信箱。  <br/> |
 ...
 **問：如果通訊群組的成員隱藏在組織的全域通訊清單（GAL）中，會發生什麼事？**
