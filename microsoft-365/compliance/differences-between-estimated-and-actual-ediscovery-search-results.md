...
- 安全性 & 規範中心的內容搜尋  <br/>  
- 在 Exchange 系統管理中心中 In-Place eDiscovery (EAC)   <br/>  
- SharePoint Online 中的 eDiscovery 中心  <br/> 
   
當您執行 eDiscovery 搜尋時，您所使用的工具會傳回專案數目 (及其符合搜尋準則的總大小) 估計值。 例如，當您在安全性 & 合規性中心執行搜尋時，預估的搜尋結果會顯示在所選搜尋的 [詳細資料] 窗格中。
...
- 未**編制索引的專案**。 未編制索引的搜尋專案可能會造成預估和實際搜尋結果之間的差異。 例如，在 Exchange 中 In-Place eDiscovery，且 SharePoint 中的 eDiscovery 中心未包含未編制索引的專案 () 執行搜尋以估計搜尋結果時，不符合搜尋準則。 不過，您可以在匯出搜尋結果時包含未編制索引的專案。 如果您在匯出搜尋結果時包含未編制索引的專案，則可能會有更多匯出的專案。 這會造成預估和匯出的搜尋結果之間的差異。
...
在 SharePoint Online 的 eDiscovery Center 中，您可以選擇在匯出 eDiscovery 搜尋的結果時，包含 Exchange 和 SharePoint) 中的未編制索引的內容 (。 若要執行此動作，請選取 [ **包含已加密或具有無法辨識格式的專案** ] 選項。 未編制索引的專案 (也稱為 uncrawlable 在 SharePoint 中) 為 Exchange 中的專案，而 SharePoint 由於某些原因未針對搜尋編制索引。 在您匯出搜尋結果時所包含的 **Exchange 索引錯誤** 報告中，會列出未編制索引的 exchange 專案。 同樣地，未編制索引的 SharePoint 專案會列在 **SharePoint 索引錯誤** 報告中。 當您匯出未編制索引的專案時，他們會下載至名為 **Uncrawlable**的資料夾。 未編制索引的 Exchange 專案包含在 PST 檔案中;每個來自 SharePoint 的未編制索引的檔也會下載。 當每個索引錯誤報表中列出任何) 時， (未編制索引的專案數。 報告中未編制索引的專案數目應符合所下載之未編制索引的專案數目。
