...
- **搜尋估計中未包含 SharePoint 和 OneDrive 中未編制索引的專案**。 來自 SharePoint 網站的未編制索引的專案和商務帳戶 OneDrive 不會包含在預估的搜尋結果中。 這是因為 SharePoint 索引不包含未編制索引之專案的資料。 只有信箱中未編制索引的專案會包含在搜尋估計中。 不過，如果您在匯出搜尋結果時包含未編制索引的專案，則會包括 SharePoint 和 OneDrive 中未編制索引的專案。 這可能會造成估計結果之間的差異 (，而不會在 SharePoint 和 OneDrive 網站中包含未編制索引的專案) 以及所下載的實際專案。 只從包含符合搜尋準則之專案的內容位置匯出未編制索引的專案的規則，在此情況下仍然適用。
