...
1. 若要查看特定使用者帳戶的詳細資料（例如，屬性值和指派的區段），請使用**InformationBarrierRecipientStatus 指令程式**搭配 Identity 參數。 
...
2. 決定您要編輯的使用者帳戶設定檔的屬性。 如需詳細資訊，請參閱[資訊障礙原則的屬性](information-barriers-attributes.md)。 
...
1. 若要查看所有現有的區段，請使用**OrganizationSegment** Cmdlet。
    ...
    > [!TIP]
    > 列印或儲存區段清單以供日後參考。 例如，如果您想要編輯某個片段，您必須知道其名稱或識別值（這會搭配 Identity 參數使用）。
...
3. 當您完成編輯原則時，請務必套用您的變更。 （請參閱[Apply 資訊屏障原則](information-barriers-policies.md#part-3-apply-information-barrier-policies)。）
...
1. 若要查看目前資訊屏障原則的清單，請使用**InformationBarrierPolicy** Cmdlet。
    ...
    在結果清單中，識別您要變更（或移除）的原則。 請注意原則的 GUID 和名稱。
...
3. 若要套用您的變更，請使用**InformationBarrierPoliciesApplication** Cmdlet。
    ...
    為您的組織套用變更的使用者。 如果您的組織很大，可能需要24小時（或更多）才能完成此程式。 （一般的指導方針是花一個小時來處理5000使用者帳戶。）

此時，一或多項資訊障礙原則會設定為非使用中狀態。 您可以從這裡執行下列其中一項動作：
- 使其保持原樣（原則設定為非使用中狀態，對使用者不會有任何影響）
- [編輯原則](#edit-a-policy) 
...
4. 當您完成移除原則時，請套用您的變更。 若要這麼做，請使用**InformationBarrierPoliciesApplication** Cmdlet。
    ...
    為您的組織套用變更的使用者。 如果您的組織很大，可能需要24小時（或更多）才能完成此程式。
