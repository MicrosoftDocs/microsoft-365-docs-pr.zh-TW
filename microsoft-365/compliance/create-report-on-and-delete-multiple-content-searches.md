...
- 若要為您組織中您可以新增至 CSV 檔案的商務用 OneDrive 網站收集 URLs 清單，請參閱[建立組織中所有 OneDrive 位置的清單](/onedrive/list-onedrive-urls)。
...
您在此步驟中建立 (CSV) 檔案的逗號分隔值，會為每個要搜尋的使用者包含一列。 您可以搜尋使用者的 Exchange Online 信箱 (包含封存信箱（如果已啟用）) 及其商務用 OneDrive 網站。 您也可以只搜尋信箱或商務用 OneDrive 網站。 您也可以搜尋 SharePoint Online 組織中的任何網站。 您在步驟3中執行的腳本會為 CSV 檔案中的每一列建立個別的搜尋。

1. 使用記事本複製並貼上下列文字至 .txt 檔案。 將此檔案儲存到本機電腦上的資料夾。 您也可以將其他腳本儲存至此資料夾。
...
2. 在 Excel 中開啟 .txt 檔案，然後使用下表中的資訊，編輯包含每個搜尋資訊的檔案。
   ...
   ****
   ...
   |`SharePointLocation`|使用者商務用 OneDrive 網站的 url，或組織中任何網站的 url。 如需商務用 OneDrive 網站的 URL，請使用下列格式： ` https://<your organization>-my.sharepoint.com/personal/<user alias>_<your organization>_onmicrosoft_com ` 。 例如，  `https://contoso-my.sharepoint.com/personal/sarad_contoso_onmicrosoft_com`。|
   |`ContentMatchQuery`|搜尋的搜尋查詢。 如需建立搜尋查詢的詳細資訊，請參閱 [內容搜尋的關鍵字查詢和搜尋條件](keyword-queries-and-search-conditions.md)。|
   |`StartDate`|若為電子郵件，收件者或傳送者傳送郵件的日期或後的日期。 在 SharePoint 或商務用 OneDrive 網站上的檔，上次修改檔的日期或之後。|
   |`EndDate`|電子郵件，由使用者傳送的郵件傳送日期或之前的日期。 在 SharePoint 或商務用 OneDrive 網站上的檔，上次修改檔的日期或之前的日期。|
   |

3. 將 Excel 檔案當做 CSV 檔儲存至本機電腦上的資料夾。 您在步驟3中建立的腳本會使用此 CSV 檔案中的資訊來建立搜尋。
...
- **Csv** 檔案-您在步驟1中建立的 CSV 檔案名。 請務必包含 [使用完整檔案名]，包含 .csv 副檔名;例如，  `ContosoCase.csv` 。

若要執行指令碼，請執行下列步驟：

1. 使用 .ps1 的檔案名尾碼，將下列文字儲存至 Windows PowerShell 腳本檔案;例如， `CreateSearches.ps1` 。 將檔案儲存到儲存其他檔案所在的相同資料夾。
...
4. 在 [ **來源 CSV** 檔案] 提示字元處，輸入 CSV 檔案名（包括 .csv 副檔名）; 否則會出現檔案名。例如，  `ContosoCase.csv` 。
...
1. 使用 .ps1 的檔案名尾碼，將下列文字儲存至 Windows PowerShell 腳本檔案;例如， `SearchReport.ps1` 。 將檔案儲存到儲存其他檔案所在的相同資料夾。
...
4. 在檔案路徑上，將 **報告儲存至 CSV 檔案 (保留空白以顯示報告)** 提示，請輸入完整檔案名路徑的檔案名 (包含 .csv 副檔名的檔案名) 如果想要將報告儲存至 CSV 檔案。 CSV 檔案名（包括 .csv 副檔名）的名稱。 例如，您可以輸入將  `ContosoCaseReport.csv` 其儲存至目前的目錄，或輸入將  `C:\Users\admin\OneDrive for Business\ContosoCase\ContosoCaseReport.csv` 其儲存至不同的資料夾。 您也可以將提示保留空白，以顯示報表，但不會將其儲存至檔案。
...
1. 使用 .ps1 的檔案名尾碼，將下列文字儲存至 Windows PowerShell 腳本檔案;例如， `DeleteSearches.ps1` 。 將檔案儲存到儲存其他檔案所在的相同資料夾。
