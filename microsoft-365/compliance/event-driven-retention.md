...
當您保留內容時，保留期間通常是依據內容的存留期 - 例如，您在文件建立之後保留 7 年，然後刪除。但是使用 Microsoft 365 中的保留標籤，您也可以讓發生特定類型事件的時間作為保留期間的依據。事件會觸發啟動保留期間，有保留標籤套用至該類型事件的所有內容，都會強制執行標籤的保留動作。
...
- 根據事件的保留標籤通常會在保留期間結束時觸發處置檢閱，讓記錄管理員可以手動檢查並處置內容。如需詳細資訊，請參閱[內容處置](disposition.md) (英文)。
    
根據事件的保留標籤具有與 Microsoft 365 中的任何保留標籤相同的功能。若要深入了解，請參閱[標籤概觀](labels.md)。
...
### <a name="step-3-publish-the-event-based-retention-labels"></a>步驟3：發佈事件型保留標籤

發佈您的事件型保留標籤，以便將其手動套用到內容。 您無法針對自動套用原則選取事件型保留標籤。 

若要發佈事件型保留標籤，請移至 [分類]**** > [保留標籤]**** 頁面。
