...
如果 [清單模式] 設為 **[允許]** ，則使用者可以將敏感性專案* *_[僅]_* _上傳到那些網域，而不可將存取權上傳到其他網域。

### <a name="unallowed-apps"></a>不受允許的應用程式

當一項原則的 _ *不受允許的應用程式和瀏覽器之存取* * 設定已開啟，且使用者嘗試使用這些應用程式存取受保護的檔案時，系統可以允許、封鎖或在封鎖活動下，但使用者仍可以覆寫限制。 所有活動都會經過稽核，並可在活動瀏覽器中查看。
...
這些案例要求你已有上線裝置，並會向 [活動瀏覽器] 進行回報。 如果您還沒有上線裝置，請參閱 [開始使用端點資料外洩防護（預覽版）](endpoint-dlp-getting-started.md)。
...
2. 選擇您在案例1中建立的 **美國個人識別資訊（PII）資料** 原則 。

3. 選擇 **編輯原則（預覽）** 。

4. 移至 **[進階 DLP 規則]** 頁面，然後編輯 **偵測到少量的美國個人識別資訊** 。
