...
在您開始使用之前，您必須設定套用到所有裝置的 DLP 原則之 DLP 設定。如果您想要建立可強制執行以下內容的原則，則您必須進行設定：
...
您可能想在裝置上從 DLP 監控、DLP 警示和 DLP 原則強制執行中排除某些太吵雜或不包含您感興趣的檔案之路徑。這些位置中的檔案不會被稽核，且在那些位置中所建立或修改的任何檔案，都不會受到 DLP 原則強制執行的制約。您可以在 DLP 設定中設定路徑排除。
...
您可以在此清單中新增在強制執行端點 DLP 原則雲端上傳存取限制時，Edge Chromium 會涉及的網域。 

如果 [清單模式] 設為 **[封鎖]** ，則使用者無法將敏感性項目上傳到這些網域。當由於項目符合 DLP 原則而封鎖上傳動作時，DLP 將會產生警告或封鎖敏感性項目的上傳。

如果 [清單模式] 設為 **[允許]** ，則使用者 * *_只_* _ 能將敏感性項目上傳到那些網域，而不可將存取權上傳到其他網域。

### <a name="unallowed-apps"></a>不受允許的應用程式

當一項原則的 _ *不受允許的應用程式和瀏覽器之存取* * 設定已開啟，且使用者嘗試使用這些應用程式存取受保護的檔案時，系統可以允許、封鎖或在封鎖活動下，但使用者仍可以覆寫限制。所有活動都會經過稽核，並可在活動總管中檢閱。

### <a name="unallowed-browsers"></a>不受允許的瀏覽器

您新增的瀏覽器是由其可執行檔名稱所標識，這些瀏覽器將無法存取符合強制執行 DLP 原則的檔案，其中的 [上傳到雲端服務限制] 已設定為 [封鎖] 或 [封鎖覆寫]。當這些瀏覽器被禁止存取檔案時，終端使用者會看到快顯通知，要求其透過 Edge Chromium 開啟檔案。
...
使用端點 DLP 和 Edge Chromium 網頁瀏覽器，您可以對不受允許的雲端應用程式和服務限制不願共用敏感性項目。Edge Chromium 瞭解，項目受端點 DLP 原則限制，並強制執行存取限制的狀況。

當您在正確設定的 DLP 原則和 Edge Chromium 瀏覽器中使用端點 DLP 做為位置時，您在這些設定中定義的不受允許的瀏覽器將無法存取符合您 DLP 原則控制項的敏感性項目。相反地，使用者會被重新導向以使用 Edge Chromium，而 Edge Chromium 則根據其對 DLP 的強加限制之理解，可在 DLP 原則中的條件符合時封鎖或限制活動。
...
> [!IMPORTANT]
> 這些端點 DLP 案例不是建立和調整 DLP 原則的正式程式。當您需要在一般情況下使用 DLP 原則時，請參閱下列主題：
...
### <a name="scenario-1-create-a-policy-from-a-template-audit-only"></a>案例 1：從範本建立原則，僅限稽核

這些案例要求你已有上線裝置，並向 [活動總管] 進行回報。如果您沒有上線裝置，請參閱[開始使用端點資料外洩防護](endpoint-dlp-getting-started.md)。
...
4. 將 **[裝置]** 以外所有位置的 **[狀態]** 欄位切換為 [關閉]。選擇 **[下一步]** 。

5. 接受預設 **[查看並自訂範本中的設定]** 選擇項目，然後選擇 **[下一步]** 。
...
11. 在 [活動總管] 中檢查受監控端點的資料。設定裝置的位置篩選並新增原則，然後依原則名稱篩選，以查看此原則的影響。如有需要，請參閱 [開始使用活動總管](data-classification-activity-explorer.md)。
...
5. 向下移至 **[事件報告]** 區段，並設定 **[當規則符合時，傳送警示給系統管理員]** 為 **[開啟]** 。電子郵件警示將會自動傳送給系統管理員，以及您新增至收件者清單中的任何人。 
   ...
   > [!div class="mx-imgBorder"]
   > ![開啟事件報告](../media/endpoint-dlp-2-using-dlp-incident-reports.png)
