...
如果 [清單模式] 設為 **[允許]**，則使用者可以將敏感性專案 ***[僅]*** 上傳到那些網域，而不可將存取權上傳到其他網域。
...
您新增的瀏覽器是由其程式名稱所標識，這些瀏覽器將無法存取符合強制執行的 DLP 原則的檔案（其中的 [上傳到雲端服務限制] 已設定為 [封鎖] 或 [封鎖覆寫]）。 當這些瀏覽器被禁止存取檔案時，終端使用者會看到快顯通知，要求其透過 Edge Chromium 開啟檔案。
