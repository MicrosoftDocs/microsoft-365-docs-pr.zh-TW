...
使用 Microsoft 365 規範中心內的 Veritas 連接器，將資料從 Pivot 平臺匯入至您的 Microsoft 365 組織中的使用者信箱。 Veritas 為您提供了一個 [資料透視](https://globanet.com/pivot/) 連接器，可設定為定期從協力廠商資料來源捕獲專案 () ，然後將這些專案匯入至 Microsoft 365。 Pivot 是一種立即訊息平臺，可與金融市場參與者共同作業。 連接器會將聊天訊息等專案從使用者的樞紐分析表轉換為電子郵件訊息格式，然後將這些專案匯入至 Microsoft 365 中的使用者信箱。
...
3. 您在 Microsoft 365 規範中心建立的 Pivot 連接器會每天連線到 Veritas Merge1 網站，並將資料透視專案傳送至 Microsoft 雲端中的安全 Azure 儲存位置。
