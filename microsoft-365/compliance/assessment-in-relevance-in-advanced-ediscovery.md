...
進階電子文件啟用早期評估，例如，定義的問題及案例匯入的資料。 進階電子文件可讓採用方法專家進行決策，並套用至文件這些決策檢閱專案。
...
專家檢閱初始評估的一組 500 檔案之後，相關性可以判斷錯誤的重新叫用值的目前邊界。 相關性也會建議 error，以達到最佳化評估組的預設邊界。 以下為一些範例：
  
- 如果評估集已經產生的錯誤，再加上或減去 10%的邊界，相關性會建議移至訓練 （需要任何額外評定檢閱）。 
