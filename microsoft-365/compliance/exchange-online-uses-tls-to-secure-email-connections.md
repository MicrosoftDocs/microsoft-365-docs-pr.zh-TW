...
瞭解 Exchange Online 和 Microsoft 365 如何使用 (TLS) 和轉寄機密 (FS) 的傳輸層安全性，以保護電子郵件通訊。 同時提供 Microsoft 為 Exchange Online 所發行之憑證的相關資訊。
...
新的憑證是由不同的憑證授權單位單位所發出 (CA) 來自 Exchange Online 所使用的先前憑證。 因此，您可能需要執行某些動作，才能使用新的憑證。
