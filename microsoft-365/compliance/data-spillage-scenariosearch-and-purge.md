...
> [!IMPORTANT]
> 無法使用本文中的程式來刪除高級 eDiscovery 案例中的審閱集中的電子郵件專案。 這是因為「審閱」集中的專案是 live service 中的專案副本，該專案會複製並儲存于 Azure 儲存位置。 這表示它們不會由您在步驟3中建立的內容搜尋傳回。 若要刪除審閱集中的專案，您必須刪除包含該評審集的高級 eDiscovery 案例。 如需詳細資訊，請參閱[關閉或刪除高級 eDiscovery 案例](close-or-delete-case.md)。
