---
title: 在 Microsoft 365 合規性中心中指派 eDiscovery 許可權
f1.keywords:
- NOCSH
ms.author: markjjo
author: markjjo
...
description: 使用 Microsoft 365 合規性中心指派執行 eDiscovery 相關工作所需的許可權。
...
---
# <a name="assign-ediscovery-permissions-in-the-microsoft-365-compliance-center"></a>在 Microsoft 365 合規性中心中指派 eDiscovery 許可權

如果您想要讓使用者使用 Microsoft 365 合規性中心中的任何[eDiscovery 相關工具](ediscovery.md)，您必須指派適當的許可權。 若要執行此動作，最簡單的方法是在「規範中心」的 [ **許可權** ] 頁面上，將適當的角色群組新增至人員。 本主題說明執行 eDiscovery 工作所需的許可權。
  
Microsoft 365 合規性中心中主要的 eDiscovery 相關角色群組稱為 **eDiscovery 管理員**。 此角色群組內有兩個子群組。
...
- **電子文件探索系統管理員** - 電子文件探索系統管理員是電子文件探索管理員角色群組的成員，而且可以和電子文件探索管理員一樣，執行相同的內容搜尋和案例管理相關工作。 此外，電子文件探索系統管理員還可以︰
  ...
  - 存取 Microsoft 365 合規性中心中 **核心 eDiscovery** 和 **Advanced eDiscovery** 頁面上列出的所有案例。

  - 針對組織中任何案例，在進階電子文件探索中存取案例資料。
...
> [!NOTE]
> 若要使用 Advanced eDiscovery 來分析使用者的資料，使用者必須 (資料) 指派 Office 365 E5 或 Microsoft 365 E5 授權。 或者，您可以指派 Microsoft 365 E5 合規性的使用者，或 Microsoft 365 eDiscovery 和審核附加元件授權，也可以指派 Office 365 E1 或 Office 365 或 Microsoft 365 E3 授權。 指派給案例的系統管理員、規範官員或法務人員，以及使用 Advanced eDiscovery 來收集、查看和分析資料不需要 E5 授權。 如需 Advanced eDiscovery 授權的詳細資訊，請參閱[Advanced eDiscovery 中的訂閱與授權](overview-ediscovery-20.md#subscriptions-and-licensing)。
  
## <a name="before-you-assign-permissions"></a>指派許可權之前

- 您必須是「組織管理」角色群組的成員，或被指派角色管理角色，才能在 Microsoft 365 合規性中心中指派 eDiscovery 許可權。
...
2. 在 Microsoft 365 合規性中心的左窗格中，選取 [**許可權**]。
...
在 Microsoft 365 合規性中心中的 [**許可權**] 頁面上，您也可以將使用者新增至合規性管理員、組織管理及檢閱者角色群組，將其指派給使用者 eDiscovery 相關的許可權。 如需指派給每個角色群組的 eDiscovery 相關 RBAC 角色的說明，請參閱 [與 ediscovery 相關的 RBAC 角色](#rbac-roles-related-to-ediscovery)。

## <a name="rbac-roles-related-to-ediscovery"></a>EDiscovery 相關的 RBAC 角色

下表列出 Microsoft 365 合規性中心中的 eDiscovery 相關 RBAC 角色，並指出預設會指派每個角色的內建角色群組。
...
此角色可讓使用者建立、編輯、刪除及控制 Microsoft 365 合規性中心中核心 eDiscovery 和 Advanced eDiscovery 案例的存取權。 如先前所述，必須先將案例管理角色指派給使用者，才能使用 **eDiscoveryCaseAdmin** 指令程式，讓他們成為 eDiscovery 管理員。
...
此角色可讓使用者在 Microsoft 365 合規性中心中執行「內容搜尋」工具，以搜尋信箱和公用資料夾、SharePoint 線上網站、商務用 OneDrive 網站、商務用 Skype 交談、Microsoft 365 群組和 Microsoft Teams，以及 Yammer 群組。 此角色可讓使用者取得搜尋結果的預估，並建立匯出報告，但需要其他角色才能啟動內容搜尋動作，例如預覽、匯出或刪除搜尋結果。

在 [內容搜尋] 和 [核心 eDiscovery] 中，獲指派符合性搜尋角色但沒有「預覽」角色的使用者可以預覽已被指派「預覽」角色的使用者在其中啟動預覽動作的搜尋結果。 沒有「預覽」角色的使用者可以在建立初始預覽動作後的兩周內預覽結果。

同樣地，在內容搜尋與核心 eDiscovery 中的使用者，會被指派符合性搜尋角色，但沒有匯出角色可以下載已獲指派「匯出」角色之使用者在其中啟動匯出動作的搜尋結果。 不含「匯出」角色的使用者可以在建立初始匯出動作後，下載兩周的搜尋結果。 之後，除非使用「匯出」角色的人重新開機匯出，否則他們無法下載結果。

預覽及匯出搜尋 (結果的兩周寬限期，但沒有對應的搜尋和匯出角色) 不會適用于 Advanced eDiscovery。 使用者必須被指派「預覽」和「匯出」角色，才能在 Advanced eDiscovery 中預覽及匯出內容。
...
此角色可讓使用者在[Advanced eDiscovery](overview-ediscovery-20.md)中存取審閱集。 獲指派此角色的使用者可以查看並開啟「 **eDiscovery > 高級**」頁面上的案例清單，其為其成員 Microsoft 365 合規性中心。 使用者存取 Advanced eDiscovery 案例後，即可選取 [**檢查集合**] 以存取案例資料。 此角色不允許使用者預覽與案例相關聯的集合搜尋結果，或執行其他搜尋或案例管理工作。 具有此角色的使用者只可以存取審閱集中的資料。
