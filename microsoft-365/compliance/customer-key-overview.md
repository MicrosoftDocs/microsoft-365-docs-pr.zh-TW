...
Microsoft 365 提供使用 BitLocker 和分散式金鑰管理員（DKM）來啟用基準、磁片區層級加密。 Microsoft 365 在應用層級為您的內容提供額外的加密層級。 此內容包含 Exchange Online 中的資料、商務用 Skype、SharePoint 線上、商務 OneDrive，以及小組檔案。 這個新增的加密層稱為「服務加密」。
...
客戶金鑰是以服務加密為基礎，可讓您提供和控制加密金鑰。 然後，Microsoft 365 會使用這些金鑰來加密您的資料，如[線上服務條款（OST）](https://www.microsoft.com/licensing/product-licensing/products.aspx)中所述。 客戶金鑰可協助您符合法規遵從性義務，因為您可以控制 Microsoft 365 用來加密及解密資料的加密金鑰。
