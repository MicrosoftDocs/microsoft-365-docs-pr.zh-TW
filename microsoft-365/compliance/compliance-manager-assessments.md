...
合規性管理員可協助您建立評估，以評估適用于貴組織的行業及地區規章的相容性。 評估是以評估範本的架構為基礎，其中包含必要的控制項、改進動作，以及完成評估的 Microsoft 動作。 為您的組織設定最相關的評估，可協助您實施原則和操作程式，以限制法規遵從性風險。
...
當相同群組中的兩個不同評估共用您管理的「改進」動作時，您對動作的執行詳細資料或狀態所做的任何更新都會自動同步整個群組。 此同步處理可讓您同時執行一個改進動作，並同時滿足數項需求。
