...
本文包含逐步程式，用以部署使用 Office 365 匯入服務的連接器，將資料從 Facebook 商務頁面匯入 Microsoft 365。 如需此程式的高層次概述，以及部署 Facebook 連接器所需的必要條件清單，請參閱 [Set up a connector to Archive facebook data](archive-facebook-data-with-sample-connector.md)。
...
4. 註冊應用程式。 在 [重新導向 URI] 底下，選取 [應用程式類型] 下拉式清單中的 [Web]，然後輸入 URI 的方塊 <https://portal.azure.com> 。
...
2. 按一下 [ **部署至 Azure**] 後，系統會以自訂範本頁面重新導向至 azure 入口網站。 填入 **基礎** 和 **設定** 詳細資料，然後按一下 [**購買**]。
   ...
   - **tenantId：** 您在步驟 1 Azure Active Directory 中建立 Facebook 連接器應用程式之後所複製之 Microsoft 365 組織的租使用者識別碼。
...
7. 在 [ **Facebook 登** 入] 底下的左導覽窗格中，按一下 [**設定**]，然後在 [**有效的 OAuth 重定向 URIs** ] 方塊中新增 OAuth 重新導向 URI。 使用格式 **\<connectorserviceuri> /Views/FacebookOAuth**，其中 connectorserviceuri 的值是您組織的 Azure 應用程式服務 URL; 例如， `https://fbconnector.azurewebsites.net` 。
...
9. 新增 Webhooks 回呼 URL，並新增驗證權杖。 回呼 URL 的格式，請使用格式 **<connectorserviceuri> /api/FbPageWebhook**，其中 connectorserviceuri 的值是您的組織的 Azure 應用程式服務 URL; 例如 `https://fbconnector.azurewebsites.net` 。
...
1. 移至 `https://<AzureAppResourceName>.azurewebsites.net` (，其中 AzureAppResourceName 是您在步驟 4) 中命名之 Azure 應用程式資源的名稱。 例如，如果名稱是 **fbconnector**，請移至 `https://fbconnector.azurewebsites.net` 。 應用程式的首頁看起來就像下列螢幕擷取畫面：
...
2. 在 [ **Facebook 商務版頁面**] 底下的 [**資料連線器**] 頁面上，按一下 [ **View**]。
...
5. 在 [ **新增連接器應用程式的認證** ] 頁面上，輸入下列資訊，然後按一下 [ **驗證連接**]。
   ...
   - 在 [ **Azure 應用程式識別碼** ] 方塊中，輸入或貼上 Application 的值 (CLIENT) ID 也稱為您在步驟1中建立的 AAD 應用程式識別碼。
