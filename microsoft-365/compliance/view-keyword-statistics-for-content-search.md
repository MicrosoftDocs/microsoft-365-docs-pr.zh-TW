...
description: 使用搜尋統計資料功能，可在安全性 & 規範中心內，顯示及比較多個內容搜尋的統計資料。 您也可以在建立或編輯搜尋查詢時設定關鍵字清單，以取得可顯示符合每個關鍵字或關鍵字片語的專案數目的增強統計資料。
ms.openlocfilehash: 07a13ab2f183cfa9349f827cf410e417c0944869
ms.sourcegitcommit: 93e6bf1b541e22129f8c443051375d0ef1374150
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 03/13/2020
ms.locfileid: "42634121"
---
# <a name="view-keyword-statistics-for-content-search-results"></a>檢視內容搜尋結果的關鍵字統計資料

在您建立並執行內容搜尋之後，您可以查看預估搜尋結果的統計資料。 這包括搜尋結果的摘要（類似于詳細資料窗格中所顯示之估計搜尋結果的摘要）、查詢統計資料，例如具有符合搜尋查詢之專案的內容位置數目，以及內容位置的名稱。具有最相符的專案。 您可以顯示一或多個內容搜尋的統計資料。 這可讓您快速比較多個搜尋的結果，並針對搜尋查詢的有效性作出決定。
...
2. 在搜尋清單中，選取一或多個搜尋，然後按一下 [**搜尋統計資料**![搜尋統計](../media/9bf56d43-25bf-4f53-a4be-f4d55102310c.png)資料] 按鈕。
...
3. 在 [**搜尋統計資料]** 頁面上，按一下下列其中一個連結，以顯示所選搜尋的統計資料。 
    ...
    **Queries**
    ...
    - 當您搜尋信箱中的所有內容（但未指定任何關鍵字）時，實際的按鍵字查詢`size>=0`就是要傳回所有專案。 
    ...
    e. 內容位置的數目（由 * * 位置類型 * * 欄所指定），包含符合**查詢**欄中所列之搜尋查詢的專案。 
    
    f. 符合**查詢**欄中所列之搜尋查詢的專案數（從指定的內容位置）。 如先前所述，如果專案包含要搜尋之關鍵字的多個實例，它只會在此欄位中計算一次。 
    
    g. 在指定的內容位置，符合**查詢**欄中搜尋查詢的所有專案總大小。 
    ...
    **主要位置**
    ...
    c. 每個搜尋的欄都是您要顯示的統計資料。 此資料行顯示專案的數目（及總大小），以符合每個內容位置中的搜尋查詢。 請注意，當您要顯示多個搜尋的統計資料時，此欄中的「NA」表示該搜尋中並未包含內容位置。 

## <a name="get-keyword-statistics-for-content-searches"></a>取得內容搜尋的關鍵字統計資料

如先前所述，[**查詢**] 頁面會顯示搜尋查詢，以及符合查詢之專案的數目（和大小）。 如果您在建立或編輯搜尋查詢時使用關鍵字清單，您可以取得增強的統計資料，顯示與每個關鍵字或關鍵字片語相符的專案數目。 這可協助您快速識別查詢的哪些部分最為有效（也是最低）。 例如，如果關鍵字會傳回大量的專案，您可以選擇縮小關鍵字查詢以縮小搜尋結果。 您可以在建立或編輯內容搜尋時設定關鍵字清單。 
...
5. 當搜尋完成時，請在搜尋清單中進行選取，然後按一下 [**搜尋統計資料** ![搜尋統計資料](../media/9bf56d43-25bf-4f53-a4be-f4d55102310c.png)] 按鈕。 您也可以顯示和比較多個搜尋的關鍵字統計資料。
...
6. 在 [**搜尋統計資料]** 頁面上，按一下 [**查詢**]，以顯示所選搜尋的關鍵字統計資料。 
    ...
    - 完整的搜尋查詢（在**Part**欄中識別為**主要**的）和完整查詢的統計資料。 請注意，[**摘要**] 頁面上顯示的是相同的統計資料。 
