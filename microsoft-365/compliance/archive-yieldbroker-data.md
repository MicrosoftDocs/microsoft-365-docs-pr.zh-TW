...
使用 Microsoft 365 規範中心的 Veritas 連接器，將 Yieldbroker 中的資料匯入至您的 Microsoft 365 組織中的使用者信箱。 Veritas 為您提供了一個 [Yieldbroker](https://globanet.com/yieldbroker/) 連接器，可設定為從協力廠商資料來源捕獲專案，並將這些專案匯入至 Microsoft 365。 連接器會將內容從 Yieldbroker 轉換為電子郵件訊息格式，然後將這些專案匯入至 Microsoft 365 中的使用者信箱。

Yieldbroker 儲存在使用者信箱之後，您可以套用 Microsoft 365 合規性功能，例如訴訟暫止、eDiscovery、保留原則及保留標籤。 在 Microsoft 365 中使用 Yieldbroker 連接器匯入和封存資料，可協助您的組織遵守政府和法規原則。
...
3. 您在 Microsoft 365 規範中心建立的 Yieldbroker 連接器會每天連線到 Veritas Merge1 網站，並將郵件傳輸至 Microsoft 雲端中的安全 Azure 存放位置。
...
1. 在 [將 **Yieldbroker 使用者對應至 Microsoft 365 使用者** ] 頁面上，啟用 [自動使用者對應]。 Yieldbroker 專案包括稱為「 *電子郵件*」的屬性，其中包含組織中使用者的電子郵件地址。 如果連接器可以將此位址與 Microsoft 365 使用者產生關聯，這些專案就會匯入該使用者的信箱。

2. 按 **[下一步]**，複查您的設定，然後移至 [ **資料連線器** ] 頁面，以查看新連接器的匯入程式的進度。

## <a name="step-4-monitor-the-yieldbroker-connector"></a>步驟4：監控 Yieldbroker 連接器

在您建立 Yieldbroker 連接器之後，您可以在 Microsoft 365 規範中心中查看連接器狀態。
