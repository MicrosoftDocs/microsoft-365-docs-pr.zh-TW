...
> [!NOTE]
> 即將開始，Microsoft 365 電子檔探索工具將支援 SharePoint 線上和商務 OneDrive 中儲存的加密檔。 這會包含套用敏感度標籤的結果所加密的檔。

## <a name="supported-encryption-technologies"></a>支援的加密技術

Microsoft eDiscovery 工具支援以 Microsoft 加密技術加密的專案。 這些技術包括 Office 郵件加密和 Azure Rights Management。 如需 Microsoft 加密技術的相關資訊，請參閱 [encryption](encryption.md)。 不支援以協力廠商加密技術加密的內容。 這包括預覽或匯出以非 Microsoft 技術加密的內容時不支援。
