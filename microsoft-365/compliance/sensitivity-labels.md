...
> [!NOTE]
> 如果是美國政府租用戶，現在所有平台都支援敏感度標籤：
> - GCC 和 GCC High 環境：[Windows 版 Office](/officeupdates/current-channel#version-2101-january-26) 和 [Mac 版 Office](/officeupdates/release-notes-office-for-mac#feature-updates-2) 的版本資訊
> - DoD 環境：[Windows 版 Office](/officeupdates/current-channel#version-2103-march-30) 的版本資訊
>
...
敏感度標籤套用至電子郵件或文件後，該標籤的任何已設定保護設定都會對內容強制執行。您可以設定敏感度標籤，以：
...
使用子標籤，您可以將使用者在 Office 應用程式中看到的母標籤下一或多個標籤分組。比方說，在「機密」下方，您的組織可能針對該分類的特定類型使用數種不同標籤。在此範例中，母標籤「機密」只是文字標籤而沒有保護設定，且因為其具有子標籤，所以不能套用至內容。使用者必須改為選擇「機密」才能檢視子標籤，接著可選擇子標籤套用至內容。
...
- 為新文件、未標誌的電子郵件和新的容器 **指定預設標籤** (當您 [已為 Microsoft Teams、Microsoft 365 群組和 SharePoint 網站啟用敏感性標籤時](sensitivity-labels-teams-groups-sites.md))。 您可以為全部這三類項目指定相同的標籤或不同的標籤。 當您為文件指定預設標籤時，Azure 資訊保護統一標籤用戶端也會將此標籤套用至未標記的現有文件。 如果使用者的文件或電子郵件不是正確的標籤，使用者可以隨時變更預設標籤。
...
您可以將使用者包含在多個標籤原則中，該使用者即會獲得來自這些原則的所有敏感度標籤和設定。 如果來自多個原則的設定彼此衝突，則會套用優先順序最高 (位置最低) 之原則的設定。 換句話說，每個設定的最高優先順序會優先套用。

如果您沒有看到使用者或群組應有的標籤或標籤原則設定行為，請檢查敏感度標籤原則的順序。 您可能需要將這個原則向下移動。 若要重新排序標籤原則，請選取 [敏感度標籤原則] > 選擇右側的省略符號 > **[下移]** 或 **[上移]**。

![敏感度標籤原則頁面上的移動選項](../media/sensitivity-label-policy-priority.png)

> [!NOTE]
> 請記住，如果指派多個原則的使用者的設定彼此衝突，則會套用優先順序最高 (位置最低) 之原則的設定。
...
若要了解如何使用敏感度標籤以符合資料隱私權法規，請參閱 [使用 Microsoft 365 部署資料隱私權法規的資訊保護](../solutions/information-protection-deploy.md) (aka.ms/m365dataprivacy)。