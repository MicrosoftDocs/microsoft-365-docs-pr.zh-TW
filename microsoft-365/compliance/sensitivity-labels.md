...
您可以將使用者包含在多個標籤原則中，該使用者即會獲得來自這些原則的所有敏感度標籤和設定。 如果來自多個原則的設定彼此衝突，則會套用優先順序最高之原則的設定。 換句話說，每個設定的最高優先順序會優先套用。

如果您沒有看到使用者或群組應有的標籤或標籤原則設定行為，請檢查敏感度標籤原則的順序。 若要重新排序標籤原則，請選取 [敏感度標籤原則] > 選擇右側的省略符號 > **[下移]** 或 **[上移]**。
