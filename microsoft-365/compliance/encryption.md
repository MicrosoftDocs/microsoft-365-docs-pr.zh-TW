...
加密程式會將您的資料（稱為明文）編碼成密文。 不像純文字，使用者或電腦不能使用密文，除非解密密文，否則會解密。 解密需要只有授權使用者才能使用的加密金鑰。 加密可協助確保只有授權的收件者才能解密您的內容。 內容包含檔、電子郵件、行事曆專案等等。
  
加密本身不會妨礙內容攔截。 加密是組織較大的資訊保護原則的一部分。 使用加密，有助於確保只有授權的協力廠商可以使用加密的資料。
  
您可以同時有多個層級的加密。 例如，您可以加密電子郵件訊息，也可以加密透過電子郵件流向的通訊通道。 使用 Office 365，您的資料會在靜止時加密，並在傳輸過程中使用數種強大的加密通訊協定，以及包含傳輸層安全性/安全通訊端階層（TLS/SSL）的技術，網際協定安全性（IPSec）和進階加密標準（AES）。
