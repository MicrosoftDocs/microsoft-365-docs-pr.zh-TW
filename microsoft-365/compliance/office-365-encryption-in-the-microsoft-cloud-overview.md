...
Microsoft enterprise cloud services 中的客戶資料受到數種技術和程式的保護，包含各種形式的加密。  (本檔中的客戶資料包含 Exchange Online 信箱內容、電子郵件內文、行事曆專案，以及電子郵件附件的內容。而且，如果適用，商務用 Skype 內容) 、SharePoint 的線上網站內容和儲存在網站中的檔案，以及上傳至商務用 OneDrive 或商務用 Skype 的檔案。 ) Microsoft 會跨其產品和服務使用多個加密方法、通訊協定和密碼，以協助提供客戶資料的安全路徑，以透過我們的雲端服務，以及協助保護儲存在雲端服務內之客戶資料的機密性 Microsoft 使用一些最強大且最安全的加密通訊協定，以防範未經授權的存取客戶資料的屏障。 適當的金鑰管理也是加密最佳作法的重要要素，而 Microsoft 可確保所有的 Microsoft 受管理加密金鑰都受到適當的保護。
...
Microsoft 提供的服務端技術會在靜止和傳輸過程中加密客戶資料。 例如，針對靜態客戶資料，Microsoft Azure 會使用[BitLocker](/windows/device-security/bitlocker/bitlocker-overview)和[DM Crypt](https://en.wikipedia.org/wiki/Dm-crypt)，Microsoft 365 使用 BitLocker、 [Azure 儲存體服務加密](/azure/)、[分散式金鑰管理員](./exchange-online-secures-email-secrets.md) (DKM) ，以及 Microsoft 365 服務加密。 針對 transport 中的客戶資料，Azure，Office 365，microsoft 商業支援，microsoft Dynamics 365，microsoft Power BI，及 Visual Studio Team Services 使用業界標準的安全傳輸通訊協定，例如網際網路通訊協定安全性 (IPsec) 和傳輸層安全性 (TLS) ，在 Microsoft 資料中心之間和使用者裝置與 Microsoft 資料中心之間。
...
Microsoft 會在每個新版本的 Windows 作業系統上，證明雲端服務中所用的基礎加密模組：

- Azure 和 Azure 美國政府版
- Dynamics 365 和 Dynamics 365 美國政府
- Office 365、Office 365 美國政府和 Office 365 美國政府國防版

靜態客戶資料的加密是由多種服務端技術所提供，包括 BitLocker、DKM、Azure 儲存體服務加密，以及 Exchange Online、商務用 Skype、商務用 OneDrive 和 SharePoint 的服務加密。 Office 365 服務加密包含的選項，可使用儲存在 Azure Key Vault 中客戶管理的加密金鑰。 此客戶管理的金鑰選項（稱為「[客戶金鑰](./customer-key-overview.md)」）可用於 Exchange Online、SharePoint Online、商務用 Skype 和商務用 OneDrive。

針對傳輸中的客戶資料，所有的 Office 365 伺服器都會以 TLS 為單位，使用 TLS 來協商安全會話，以確保客戶資料的安全。 例如，Office 365 會協商安全會話，以在 web、行動用戶端和網頁瀏覽器上商務用 Skype、Outlook 和 Outlook。
...
- [Office 365商務用 Skype、商務用 OneDrive、SharePoint 線上及 Exchange Online 的加密](/compliance/assurance/assurance-encryption-for-microsoft-365-services) 
