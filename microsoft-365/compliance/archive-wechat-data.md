...
---
# <a name="set-up-a-connector-to-archive-wechat-data-preview"></a>設定 WeChat 資料 (預覽的連接器) 

使用 Microsoft 365 規範中心內的 TeleMessage 連接器，匯入及封存 WeChat 和 WeCom 通話、聊天、附件、檔案及召回的郵件。 在您設定及設定連接器之後，它會連線到您組織的 TeleMessage 帳戶，並使用 TeleMessage WeChat 歸檔人員，匯入 Microsoft 365 中信箱的員工行動通訊。
...
3. 您在「Microsoft 365 規範中心」中建立的 WeChat 歸檔器連接器每天會連線至 TeleMessage 網站，並將電子郵件訊息從過去24小時內傳送至 Microsoft 雲端中的安全 Azure 儲存體區域。
...
請遵循本節中的步驟，在「Microsoft 365 規範中心」中建立 WeChat 的歸檔器連接器。 連接器會使用您提供的資訊來連線至 TeleMessage 網站，並將 WeChat 通訊資料傳送至 Microsoft 365 中對應的使用者信箱。
