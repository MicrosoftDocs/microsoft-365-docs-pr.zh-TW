...
---
# <a name="define-case-and-tenant-settings-in-advanced-ediscovery-classic"></a>在 [Advanced eDiscovery (傳統) 中定義案例與租使用者設定
...
事件記錄會在高級 eDiscovery 作業期間提供有關高級 eDiscovery 處理的中繼資料。 例如，它包含主要的高級 eDiscovery 程式的開始時間 (匯入、分析、相關性及匯出) 以及結束時間和狀態。 此記錄可用於追蹤及疑難排解資料處理活動，以及用於處理錯誤和警告。
