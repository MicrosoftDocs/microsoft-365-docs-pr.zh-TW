...
使用 Microsoft 365 規範中心內的 Veritas 連接器，將 FX Connect 共同作業平臺的資料匯入和封存至您的 Microsoft 365 組織中的使用者信箱。 Veritas 會提供 [Fx 連接](https://globanet.com/fx-connect/) 連接器，設定為捕獲 FX 連線專案，並將這些專案匯入 Microsoft 365。 連接器會將內容從 FX 連線（例如交易、郵件，以及組織的 FX 線上帳戶中的其他詳細資料）轉換為電子郵件訊息格式，然後再將這些專案匯入 Microsoft 365 中的使用者信箱。

將 FX Connect 資料儲存在使用者信箱之後，您可以套用 Microsoft 365 合規性功能，例如訴訟暫止、eDiscovery、保留原則和保留標籤，以及通訊法規遵從性。 使用 FX Connect connector 在 Microsoft 365 中匯入和封存資料，可協助您的組織遵守政府和法規原則。

## <a name="overview-of-archiving-fx-connect-data"></a>封存 FX 連接資料的概覽

下列概要說明如何使用連接器封存 Microsoft 365 中的 FX 連接資訊。

![FX 連接資料的封存工作流程](../media/FXConnectConnectorWorkflow.png)

1. 您的組織可以搭配 FX Connect 來設定和設定 FX 連線網站。

2. 每24小時一次，來自 FX Connect 帳戶的專案會複製到 Veritas Merge1 網站。 連接器也會將 FX 連接專案轉換為電子郵件訊息格式。

3. 您在 Microsoft 365 規範中心建立的 FX 連線連接器，會每天連線到 Veritas Merge1 網站，並將 FX 連線專案傳送至 Microsoft 雲端中的安全 Azure 儲存位置。

4. 連接器會使用自動使用者對應的 *電子郵件* 屬性值，將專案匯入至特定使用者的信箱，如 [步驟 3](#step-3-map-users-and-complete-the-connector-setup)所述。 在使用者信箱中建立名為 **FX Connect** 的 [收件匣] 資料夾中的子資料夾，並將這些專案匯入該資料夾。 連接器會使用 *Email* 屬性的值來執行此動作。 每個 FX Connect 專案都包含此屬性，它會填入專案的每個參與者的電子郵件地址。
...
## <a name="step-2-configure-the-fx-connect-connector-on-the-veritas-merge1-site"></a>步驟2：在 Veritas Merge1 網站上設定 FX 連接連接器
...
在您建立 FX 連線連接器之後，您可以在 Microsoft 365 規範中心中查看連接器狀態。
