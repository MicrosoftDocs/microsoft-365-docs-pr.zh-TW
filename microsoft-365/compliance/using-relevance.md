...
---
# <a name="use-the-relevance-module-to-analyze-data-in-advanced-ediscovery"></a>使用相關性模組分析高級 eDiscovery 中的資料

在 [Advanced eDiscovery] 中，相關性模組包含與案例相關的相關性訓練和評論。 若要使用相關性工作流程，請移至 [管理審閱集內的審閱集]，然後按一下 [顯示相關性]。 您必須先完成一些步驟，才能開始工作流程：

- 處理常式：新增至審閱集合的每個負載集會在這裡顯示為 "容器"。 您必須先處理這些檔，才能將其新增至相關性模組;您也可以在其中將其標記為植入或預先標記特定問題的位置。

- 新增至相關性：在 [相關性載入] 底下 \> ，您可以新增已處理成相關性的檔，讓他們可以進行訓練。
...
- **決定**：在批次計算後會顯示套用至整個案例的分析結果，並顯示用於進行檔檢查決策的資料。
    
- **測試**：可以測試結果，以確認高級 eDiscovery 處理的有效性和效能。

- **搜尋**：完成相關性工作流程之後，您可以在您的複查集內執行查詢時，針對您的問題，使用此輸出（如，讀取檔的百分點）。
...
- **標記與訓練**： 
  ...
  - 只有在必要時才使用 [ **略過標記** ] 選項。 「高級 eDiscovery」不會根據略過的檔案進行訓練。 在評估中，如果您很難判斷檔案是否相關，最好在可能的情況下，標記為相關 (R) 或不相關 (NR) ，而不是選取 [ **略過**]。 當 [Advanced eDiscovery] 評估訓練時，可看到這些檔案類型的處理方式。
...
- **問題和採樣模式**：當工作完成時，通常會關閉問題 (相關性訓練是「穩定」和「批次計算」已執行) 、當問題取消時，或其他使用者正在處理問題時）。
