...
1. 在 [**相關性 \> **標籤] 索引標籤中，預設會在左窗格中顯示 [標記] 窗格，並顯示範例檔案，一次顯示一個標記。 
    ...
    **標記**標籤標記統計資料會顯示檔案範例號碼（位於左窗格上方）、目前顯示的檔案輸出範例中的總檔案數目（右窗格的底部），以及範例中已標記檔案（左窗格底部）的目前總數目，這會隨著您的標記檔而變更。 這適用于任何已完成的相關性標記，不論是在評估、訓練、追趕或測試中。 
...
|略過  <br/> |略過  <br/> |C  <br/> |Shift + A  <br/> |
  ...
  - **大量**標籤：使用此選項，可選取 [**全部**]，為所選取的檔案設定所有問題（覆寫已標記的問題）的標記，或選取**其餘**專案將標記套用至其餘的未加標記的問題，以指定檔案的多個問題。 選取的選項會在所有此使用者的情況下保持作用，直到該使用者變更為止（設定為每位使用者的所有使用者案例）。 
...
3. 您可以透過左窗格下拉式清單中的 [**批註**] 選項，查看及編輯與檔案相關聯的任意文字批註。 選 
