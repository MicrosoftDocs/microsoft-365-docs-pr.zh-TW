...
2. 按一下 [ **連接器** ] 索引標籤，然後選取 [ **Webex 小組** 連接器] 以顯示飛出頁面。 此頁面包含連接器的屬性和資訊。
