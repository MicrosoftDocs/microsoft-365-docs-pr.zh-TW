...
使用 Microsoft 365 規範中心內的 Veritas 連接器，匯入 XIP 來源平臺的資料，並將其封存至您的 Microsoft 365 組織中的使用者信箱。 Veritas 提供 [XIP](https://globanet.com/xip/) 連接器，可讓您使用 XIP 檔案將專案匯入 Microsoft 365。 XIP 檔案類似 ZIP 檔案，但可用於使用數位簽章。 在解壓縮 XIP 來源檔案之前，Veritas 合併1會驗證數位簽章。 連接器會將 XIP 來源檔案中的內容轉換成電子郵件格式，然後將這些專案匯入至 Microsoft 365 中的使用者信箱。
...
3. 您在 Microsoft 365 規範中心建立的 XIP 連接器會每天連線到 Veritas Merge1 網站，並將郵件傳輸至 Microsoft 雲端中的安全 Azure 存放位置。
