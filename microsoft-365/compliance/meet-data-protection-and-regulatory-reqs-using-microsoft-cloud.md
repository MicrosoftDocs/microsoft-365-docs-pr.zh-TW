...
合規性管理員是 Microsoft [服務信任入口網站](./get-started-with-service-trust-portal.md)中以工作流程為基礎的風險評估工具，可讓您追蹤、指派及驗證貴組織與 Microsoft 專業服務和 Microsoft 雲端服務相關的法規遵循活動，例如 Microsoft Office 365、Microsoft Dynamics 365，以及 Microsoft Azure。
...
- **Microsoft 管理控制項** - 對於每個雲端服務，Microsoft 會實作及管理一組「控制項」作為 Microsoft 與各種標準和法規的合規性。這些控制項會組織成「控制項系列」，符合對應憑證或「評估」所符合規定的結構。對於每個 Microsoft 管理控制項，「合規性管理員」會提供 Microsoft 實作控制項的方法，以及獨立協力廠商稽核員測試及驗證實作的方式和時間。
  ...
  1. 從對應到 Microsoft 管理的控制措施之認證或規定指定下列資訊。
  ...
  1. 控制措施的合規性分數，表示與每個 Microsoft 管理的控制措施關聯的風險等級 (由於不符合合規性或控制措施失敗)。 如需詳細資訊，請參閱[了解合規性分數](#understanding-the-compliance-score)。 請注意，合規性分數的評分介於 1 到 10 並以色彩標示。 黃色表示低風險控制措施、橘色表示中度風險控制措施，而紅色則表示高風險控制措施。

  1. 某個控制措施的實作狀態相關資訊、測試控制措施的日期、執行測試的人員，以及測試結果。

  1. 針對每個控制措施，您可以按一下 **[更多]** 來查看其他資訊，包括 Microsoft 控制措施實作的相關詳細資料，以及獨立第三方稽核員已測試和驗證該實作的方式和時間的相關詳細資料。
...
|權限|合規性管理員讀取者|合規性管理員參與者|合規性管理員評定員|合規性管理員系統管理員|入口網站管理員|
|---|:---:|:---:|:---:|:---:|:---:|
|**讀取資料** - 使用者可以讀取但無法編輯資料。|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|
|**編輯資料** - 使用者可以編輯所有欄位，除了 [測試結果] 和 [測試日期] 欄位以外。||![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|
|**編輯測試結果** - 使用者可以編輯 [測試結果] 和 [測試日期] 欄位。|||![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|
|**管理評估** - 使用者可以建立、封存和刪除「評估」。||||![核取記號](../media/checkmark.png)|![核取記號](../media/checkmark.png)|
|**管理使用者** - 使用者可以在其組織中，將其他使用者新增至「讀取者」、「參與者」、「評估者」和「管理員」角色。貴組織中具有全域管理員角色的使用者才可以從入口網站管理員角色中新增或移除使用者。|||||![核取記號](../media/checkmark.png)|
|

## <a name="understanding-the-compliance-score"></a>了解合規性分數

在儀表板上，「合規性管理員」會在磚的右上角顯示 Office 365 評估的總分。這是「評估」的整體總「合規性分數」，且是「評估」中每個標示為 [已實作] 和 [已測試] 控制項評估所收到的累積分數。新增「評估」時，您會看到「合規性分數」已接近完成，因為 Microsoft 已實作、且經獨立第三方測試的 Microsoft 管理控制項都已套用。
...
顯示的控制項合規性分數值會依通過/失敗基礎「完整」套用至合規性總分--控制項實作並通過後續評估測試，或相反狀況；沒有部分實作的部分分數。僅當控制項將其 [實作狀態] 設為 [已實作] 或 [替代實作]，且 [測試結果] 設為 [通過] 時，指派的點數才會新增至合規性總分。

最重要的是，合規性分數可藉由指出在發生與控制項相關的失敗時，哪些控制項具有較大的潛在威脅，協助您安排要優先專注進行實作的控制項。除了風險優先順序之外，評估控制項與其他控制項相關時 (不論是在相同的評估內，或是相同評估群組中的另一個評估中)，成功完成單一控制項可能會大幅降低根據控制項測試結果同步處理的工作。
...
在評估中，GDPR 控制項 7.5.5 與其他 5 個控制項相關 (7.4.1、7.4.3、7.4.4、7.4.8 和 7.4.9)，每個都有 6 或 8 分的中高嚴重性風險排名分數。使用評估篩選器，我們選取了這所有的控制項，讓它們可在 [評估] 檢視中顯示，且可從下方看到，它們全都尚未評估。
...
當您建立新的評估時，系統會提示您建立指派「評估」的群組，或指派「評估」至現有的群組。群組可讓您以邏輯方式組織「評估」，並共用「評估」之間一般資訊與工作流程工作，其具有相同或相關的客戶管理控制項。
...
一旦將評估新增至評估群組後，就無法變更群組。您可以重新命名評估群組，會變更所有與該群組相關聯評估的評估群組名稱。您可以建立評估和新的評估群組，並從現有的評估複製資訊，這樣會在不同的評估群組中有效建立該評估的複本。封存評估會中斷評估與評估群組之間的關係。對其他相關評估的任何進一步更新都不會再反映於封存的評估中。

如先前所述，使用群組的主要優點之一是，當同一個群組中兩個不同「評估」共用相同的客戶管理控制權 (因此每個控制項的客戶動作都一樣) 時，在一個「評估」中完成實作詳細資料、測試資訊以及控制項的狀態，會同步處理至群組中任何其他「評估」的同一個控制項。換句話說，如果「評估」共用相同的控制項，且這些「評估」位於同一個群組，您就只須管理一個「評估」中控制項的評估程序。該控制項的結果將會自動同步到其他「評估」。例如，ISO 27001 和 ISO 27018 都有關於密碼原則的控制項。如果一個「評估」中的控制項測試狀態設定為「通過」，只要兩個評估都屬於同一個「評估」群組，其他「評估」中的控制項會更新 (並標示為「通過」)。
...
3. 按 [下一步] 並執行下列動作：
   ...
   1. 從 [選取產品] 下拉式清單中選擇要評定合規性的 Microsoft 雲端服務。

   1. 從 [選取認證] 下拉式清單中選擇要根據哪個認證評定選取的雲端服務。
...
如先前所解說，當您建立評估群組時，可以選擇將資訊從現有群組中的「評估」複製到新群組中的新「評估」。這可讓您將已完成的評估和測試工作套用到新「評估」中的相同客戶管理控制項。例如，如果您在貴組織中有所有 GDPR 相關「評估」的群組，當您將新「評估」新增至群組時，可以從現有評估工作複製一般資訊。
...
2. 此區段會顯示「評估」摘要資訊，包括「評估群組」的名稱、「產品」、「評估名稱」、評估控制項的數目

3. 本節會顯示「評估篩選器」控制項。如需如何使用「評估篩選器」控制項的更詳細說明，請參閱[管理評估程序](#managing-the-assessment-process)一節。

4. 本節會顯示評估範圍內的個別雲端服務。

5. 本節中包含 Microsoft 管理控制項。相關的控制項會依控制項系列組織。按一下控制項系列加以展開，並顯示個別的控制項。

6. 本節中包含客戶管理控制項，其也會依控制項系列組織。按一下控制項系列加以展開，並顯示個別的控制項。

7. 顯示控制項系列中的控制項總數，以及已存取這些控制項的數目。 追蹤貴組織評定客戶管理控制項的進度是合規性管理員的主要功能。 如需詳細資訊，請參閱[了解合規性分數](#understanding-the-compliance-score)一節。
...
合規性管理員採用定期更新的設計，以跟上法規要求的變更，以及雲端服務的變更。這些更新包括客戶管理控制項的變更。我們會提供變更記錄檔，協助您了解這些變更的影響，包括新增或變更之內容的詳細資料，以及變更對現有評定有何影響的指導方針。一般而言，有兩種類型的變更：
...
|GDPR|HIPAA|ISO 27001|ISO 27018|NIST 800-53|NIST 800-171|變更類型|變更描述|建議客戶執行的動作|
|---|---|---|---|---|---|---|---|---|
|6.13.2|||C.16.1.1|||主要|先前編號為 6.12.1.1。 <p> 已新增詳細資料至建議。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
||||||3.1.6|主要|已新增步驟至指導方針，包括啟用稽核及搜尋稽核記錄。|檢閱客戶動作中的更新建議。|
|6.8.2|||A.10.2|||主要|先前編號為 6.7.2.9。 <p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
|6.6.4|45 C.F.R. § 164.312(a)(2)(i) <p> 45 C.F.R. § 164.312(d)|A.9.4.2||IA-2|3.5.1|主要|先前編號為 6.5.2.3。 <p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
|6.13.1|45 C.F.R. § 164.308(a)(1)(i)|A.16.1|C.16.1|IR-4(a)|3.6.1|主要|先前編號為 6.12.1。 <p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
|6.7||||||主要|先前編號為 6.6.1.1。<p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
|6.6.5|||A.10.8|IA-3|3.5.2|主要|先前編號為 6.5.4.2。 <p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
|6.15.1||||||主要|先前編號為 6.14.1.3。 <p> 包含其他建議與動作項目的更新指導方針。|重新評估控制項：在客戶動作中檢閱更新的指導方針，然後依照實作和評估控制項的建議步驟。|
...
|5.2.1||||||次要|先前編號為 5.2.2。 <p> 已釐清指導方針內的客戶責任。|檢閱客戶動作中的更新建議。|
|6.11.1|45 C.F.R. § 164.312(e)(2)(ii)|A.10.1.1 <br> A.10.1.2 <br> A.18.1.5|C.10.1.1|SC-13|3.13.11|次要|先前編號為 6.10.1.2。 <p> 修正拼字錯誤。|不需要任何動作。|
|7.5.1||||||次要|先前編號為 A.7.4.1。 <p> 修正拼字錯誤。|不需要任何動作。|
|||A.8.2.3|||3.1.3|次要|移除不必要的句子。|不需要任何動作。|
||45 C.F.R. § 164.308(a)(4)(i)|A.6.1.2||AC-5(a)|3.1.2  <br> 3.1.4|次要|包含其他建議與動作項目的更新指導方針。|檢閱客戶動作中的更新建議。|
...
|上一個控制措施識別碼<br>(2017 年 11 月預覽)|新控制措施識別碼<br>(2018 年 2 月 GA 發行)|
|---|---|
|5.2.2|5.2.1|
|5.2.3|5.2.2|
|5.2.4|5.2.3|
