...
---
# <a name="set-up-a-connector-to-archive-ice-chat-data-preview"></a>設定連接器以封存冰激淩聊天資料（預覽）

使用 Microsoft 365 規範中心內的原生連接器，從「ICE 聊天共同作業」工具匯入及封存金融服務聊天資料。 在您設定及設定連接器之後，它會每天連線到您組織的冰激淩聊天安全 FTP （SFTP）網站，將聊天訊息的內容轉換成電子郵件格式，然後將這些專案匯入至 Microsoft 365 中的信箱。
...
- 您必須在 Exchange Online 中指派「信箱匯入匯出」角色，才能在步驟3中建立 ICE 聊天連接器（以及在步驟1中下載公開金鑰和 IP 位址的系統管理員）。 在 Microsoft 365 規範中心的 [**資料連線器**] 頁面上新增連接器時，此角色是必要的。 依預設，此角色不會指派給 Exchange Online 內的任何角色群組。 您可以將信箱匯入匯出角色新增至 Exchange Online 中的「組織管理」角色群組。 或者，您可以建立角色群組、指派信箱匯入匯出角色，然後將適當的使用者新增為成員。 如需詳細資訊，請參閱「管理 Exchange Online 中的角色群組」一文中的 [[建立角色群組](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#create-role-groups)或[修改角色群組](https://docs.microsoft.com/Exchange/permissions-exo/role-groups#modify-role-groups)] 區段。

## <a name="step-1-obtain-ssh-and-pgp-public-keys"></a>步驟1：取得 SSH 和 PGP 公開金鑰

第一步是取得安全命令介面（SSH）和非常好的隱私權（PGP）的公開金鑰複本。 您可以在步驟2中使用這些機碼來設定 ICE 聊天 SFTP 網站，以允許連接器（您在步驟3中建立）連接到 SFTP 網站，並將「ICE 聊天」資料轉移至 Microsoft 365 信箱。 您也會在此步驟中取得 IP 位址，當您設定 ICE 聊天 SFTP 網站時，您可以使用此位址。

1. 移至 [https://compliance.microsoft.com](https://compliance.microsoft.com) 並按一下左側導覽中的 [**資料連線器**]。

2. 在 [**冰激淩聊天**] 底下的 [**資料連線器（預覽）** ] 頁面上，按一下 [ **View**]。
...
5. 在 [**新增冰聊天的認證**] 頁面的 [步驟 3] 下，于下列方塊中輸入必要的資訊，然後按一下 [**驗證**連線]。
   ...
   - **SFTP URL:** ICE 聊天 SFTP 網站的 URL （例如，sftp.theice.com）。
