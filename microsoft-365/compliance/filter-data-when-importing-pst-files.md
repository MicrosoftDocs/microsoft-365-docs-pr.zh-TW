...
使用 Office 365 Import service 中的新智慧匯入功能，篩選實際匯入至目標信箱的 PST 檔案中的專案。 以下為運作方式：
  
- 在您建立並送出 PST 匯入工作之後，PST 檔案會上傳至 Microsoft 雲端中的 Azure 儲存體區域。
    
- Microsoft 365 會以安全且安全的方式來分析 PST 檔案中的資料，方式是識別信箱專案的保留天數，以及 PST 檔案中包含的不同郵件類型。
...
下圖顯示智慧匯入程式，並強調 Office 365 所執行的工作。
...
3. 針對您想要完成的匯入工作，按一下 [已**準備好匯入 Office 365** ]。 
...
5. 根據是否要裁切匯入至 Microsoft 365 的資料，在 [**是否要篩選資料？**] 下，執行下列其中一項操作：
    
    a. 按一下 **[是，我想要在匯入之前加以篩選]** ，以裁切您匯入的資料，然後按 **[下一步]**。
    
    [匯**入資料到 Office 365] 頁面**會顯示從 Microsoft 365 執行之分析的詳細資料洞察力。 
    ...
    [匯**入資料到 Office 365** ] 頁面上的資料洞察力會根據您的篩選設定進行更新，包括根據篩選設定所匯入的總數據量。 請注意，也會顯示篩選設定的摘要。 您可以按一下篩選旁邊的 [**編輯**]，以視需要變更設定。 
...
- Microsoft 365 如何判斷年齡篩選的增量？ 當 Microsoft 365 分析 PST 檔案時，它會查看每個專案的傳送或接收時間戳記（如果專案同時有傳送和接收的時間戳記，則會選取最舊的日期）。 然後，Microsoft 365 會查看該時間戳記的 year 值，並將它與目前的日期進行比較，以判斷專案的年齡。 然後，這些年齡會做為**時期**篩選的下拉式清單中的值使用。 例如，如果 PST 檔案有來自2016、2015及2014的郵件，則**Age**篩選中的值會是**1 年**、 **2 年**和**3 年**。
    
- 下表列出 [其他**選項**] [飛出] 頁面上的 [**類型**篩選] 中的 [**其他**類別] 類別中所包含的郵件類型（請參閱上一個程式中的步驟5b）。 目前，當您匯入 Pst 至 Office 365 時，無法排除 "Other" 類別中的專案。 
