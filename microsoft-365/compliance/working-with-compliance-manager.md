...
- 一旦評估已新增至評估群組中，無法變更群組。 您可以重新命名評估群組，將變更的群組該群組相關聯的所有 「 評估 」 的 「 評估名稱。 您可以建立評定和新評定群組，並且從現有評定複製資訊，這會在不同的評定群組中有效地建立該評定的重複項目。
- 封存評估會中斷該評估及群組之間的關係。 其他相關評定的任何進一步更新不會再反映於已封存評定中。
