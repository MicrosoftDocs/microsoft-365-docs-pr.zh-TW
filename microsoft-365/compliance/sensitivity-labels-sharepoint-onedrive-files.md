...
> [!NOTE]
> 若尚未使用雲端型金鑰（內部部署金鑰）套用加密，則金鑰管理拓撲通常稱為「保留您自己的金鑰」 (HYOK) ，處理檔案內容的 SharePoint 行為不會變更。
>
...
- 如果您上傳的檔案包含套用加密的靈敏度標籤，則不會移除加密，因此不支援這些檔案、合著、eDiscovery、DLP 和 search。
