...
---
# <a name="run-the-process-module-in-advanced-ediscovery-classic"></a>在高級 eDiscovery (傳統) 中執行進程模組
...
- 檔案 **識別碼：「** 高級 eDiscovery」中的唯一檔識別碼。 如果沒有匯入的檔案識別碼，高級 eDiscovery 便會自動產生識別碼。 如果您在後續的處理常式載入中對應識別碼，並且對應的路徑與初始程式負載中的不同，則 Advanced eDiscovery 會取代路徑 (，而不是新增新的檔案專案) 。 在匯出程式中，可以使用識別碼做為參考。 識別碼值不應該是 "-1"。
    
- **MD5**：此簽章是用來區分檔案 (兩個檔案不會被視為完全重複的專案，除非它們的 MD5) 相同。 根據預設，「高級 eDiscovery」會計算檔的 MD5。 當載入的檔案為文字檔時，建議您在 [高級 eDiscovery] 中載入和對應原始的 MD5 值，而不是計算它。
...
- **文字提取**：從原生檔案提取文字時，除了一般文字之外，還會提取下列專案：隱藏的文字 (excel 及 .doc) 、隱藏的資料行 (Excel) 、追蹤變更 ( .doc) 、演講者附注 ( .ppt) 、內嵌物件 (（如 .ppt) 中的 Excel 物件）。 這些可以在文字編輯器中查看。
...
- **中繼資料**：判斷是否要在案例資料庫中儲存以供日後參考的中繼資料，例如日期範圍、檔案大小、檔案類型、擁有者和主旨。 在不重新執行清查或重新處理重新處理的開銷的情況下，載入檔之後，即可載入中繼資料。 
  ...
  - 如果檔案最初是以檔識別碼載入，請在載入中繼資料時對應 [識別碼] 欄。 依路徑參照檔 (，而不是識別碼) 會導致重新載入使用不同識別碼的檔案。 「高級 eDiscovery」會建立檔案的複本，而不是載入現有檔案的中繼資料。
    
- **系列**：無法在沒有其上層 (head) 的情況下載入家族。 
    
- 檔案 **大小**：載入至 [Advanced eDiscovery] 的檔案大小不會有任何限制。 為了進行分析 (分析、相關性等 ) ，限制為5242880個字元的解壓縮文字。 較大的檔案會被忽略 (例如，在相關性中，檔案不會參與相關性訓練程式，也不會接收批次計算之後的相關性分數) 。
