...
國防 Information Systems 機關（DISA）是美國國防部（DoD）的打擊支援機關。 它可提供公司資訊基礎結構、通訊支援，以及安全、彈性的企業雲端環境，以供 DoD、白房屋，以及在美國國防中扮演角色的任何其他組織使用。

為了實施其規定，DISA 已開發 DoD Cloud 計算安全性需求指南（SRG）。 SRG 會為雲端服務提供者（Csp）定義基準的安全性需求，該服務提供者會主控 DoD 資訊、系統和應用程式，以及 DoD 使用雲端服務。 它會取代 DoD 雲端安全性模型，並對應至 DoD 風險管理架構和 NIST 800-37/53。

DoD 雲端服務支援定義 SRG 中的原則、安全性控制及其他需求，以供發行及維護。 它會指導 DoD 計畫及授權使用雲端服務提供者的代理商和部門。 雲端服務支援也會評估與 SRG 相容性相關的 CSP 產品。這是一種授權程式，Csp 可提供與 DoD 標準相容的 attestations。 它會在適當時發出 DoD 的臨時授權（PAs），所以 DoD 的代理商和支援組織可以使用雲端服務，而不需要自行完成完全核准程式，節省時間和精力。
...
DISA 雲端服務支援已授與適用于 DoD Microsoft Azure 政府的 DoD 影響等級 5 PA。 DISA 也授與 Office 365 美國政府國防 a DoD 影響層級 5 PA。 影響層級5涵蓋由法律、其他政府法規或擁有資訊的代理商所認定的控制未分類資訊（CUI），並需要比第4級提供更高的保護等級。 此外，也涵蓋未分類的全國安全性系統。

## <a name="dod-impact-level-4-provisional-authorization"></a>DoD 影響層級4的臨時授權

DISA 雲端服務支援已授與適用于 Microsoft Azure 政府的 DoD 影響層級 4 PA。 這是以複查其 FedRAMP 授權和雲端計算 SRG 所需的其他安全性控制措施為基礎。 （FedRAMP 是一種可為政府進行安全雲端計算的 US 程式。）

影響層級4涵蓋控制的未分類資訊，即需要防止未授權披露的資料，以執行順序13556（11月2010）和其他重要的資料。 它可能包括指定為僅供官方使用的資料、法律強制敏感的機密或機密的安全性資訊。 這種授權可讓美國聯邦政府客戶在範圍內的 Microsoft 政府雲端服務上部署這些類型的高度機密資料。
...
- Azure 和 Azure 政府基礎結構即服務（IaaS）和英保通（PaaS），都是根據從 FedRAMP 的聯合授權董事會進行的臨時授權（P-ATO）授與此授權。
...
- [DoD 雲端計算安全性需求指南（SRG）及其他檔](https://public.cyber.mil/dccs/dccs-documents/)
