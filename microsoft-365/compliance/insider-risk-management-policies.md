...
6. 在 **[指定優先內容 (選用)]** 頁面上，您可以指派來源，以設定危險使用者活動的優先順序：
    ...
    - 機密資訊類型： 選取 [**新增敏感資訊類型**，然後選取您想要排列優先順序敏感度類型。 例如，*[美國銀行帳戶號碼]* 和 *[信用卡號碼]*。
...
6. 在 **[使用者]** 頁面上，選取 **[新增使用者或群組]** 以定義要包含在原則中的使用者，或勾選 **[所有使用者和擁有郵件功能的群組]** 核取方塊。 選取 [**下一步**繼續
7. **指定內容設定的優先順序 （選用）** 在頁面上，更新來源來設定優先順序有風險的使用者活動：
    - SharePoint 網站： 選取 [**新增 SharePoint 網站**，選取您想要排列優先順序的 SharePoint 組織。 例如，*"group1@contoso.sharepoint.com/sites/group1"*。
    - 機密資訊類型： 選取 [**新增敏感資訊類型**，然後選取您想要排列優先順序敏感度類型。 例如，*[美國銀行帳戶號碼]* 和 *[信用卡號碼]*。
    - 敏感度標籤： 選取 [**新增敏感度標籤**，選取您想要排列優先順序的標籤。 例如，*[機密]* 和 *[密碼]*。
8. 選取 **[下一步]** 繼續。
9. 在**選擇警示指標**頁面上，您會看到包含在您為此原則選取的範本中的指標。 如果您在精靈開始時選取 *[資料外洩]* 範本，必須從 **[DLP 原則]** 下拉式清單中選取 DLP 原則。
10. 在**選取監視視窗**頁面上，定義原則的[監視視窗條件](insider-risk-management-policies.md#monitoring-windows)。 視需要設定監視視窗。
