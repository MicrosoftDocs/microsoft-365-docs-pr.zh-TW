...
自動套用此敏感度標籤時，使用者會在其 Office 應用程式中看到通知。 他們可以選擇 [確定]**** 來關閉通知。
...
Word、PowerPoint 和 Excel 支援建議的標籤。
...
- 當您儲存文件時，建議的標籤會套用至 Word、Excel 和 PowerPoint。

- 您無法針對之前已加上較高敏感性標籤的文件使用建議標籤。 如果內容已加上較高敏感性標籤，使用者將不會看到含有建議和原則提示的提示。
