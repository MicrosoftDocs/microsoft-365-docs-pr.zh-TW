...
合規性管理員儀表板會顯示針對評估每個評估磚合規性總分。 這是整體合規性分數評估，而且累積的評估中每個已實作和測試控制項接收到的點。 針對新的 「 評估，合規性分數會具有由獨立第三方測試包含 Microsoft 管理控制項的初始值。 合規性分數是根據計算 Microsoft 受管理的巨集指令分數和客戶管理動作分數。 合規性分數可協助排列優先順序的評估和控制項以專注於改善您的整體合規性狀態。

控制項的顯示合規性分數的值會套用]*其完整*總合規性分數以通過/失敗為基礎。 控制項實作並傳遞後續評估測試或沒有。 當控制項有指派的點新增到合規性分數：
