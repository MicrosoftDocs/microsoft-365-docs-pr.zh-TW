...
---
# <a name="set-up-microsoft-viva-learning-preview-in-the-teams-admin-center"></a>在 Teams 系統管理中心中設定 Microsoft Viva Learning (預覽) 

> [!NOTE]
> 本文中的資訊與在正式發行之前可能會充分修改的預覽產品有關。 

Teams 系統管理員必須執行某些步驟，為租使用者中的使用者啟用 Viva Learning (預覽) 。 根據啟用承租人的方式，這些步驟會有所不同：  [*公開預覽*](set-up-teams-admin-center.md#public-preview-tenants) 或 [*私人預覽* (或 Beta)](set-up-teams-admin-center.md#private-preview-tenants)。

## <a name="public-preview-tenants"></a>公開預覽承租人

### <a name="administrator-steps-for-public-preview-tenants"></a>公開預覽承租人的管理員步驟

由於 Viva Learning (Preview) 尚未一般可用，因此必須執行某些步驟，才能啟用特定使用者或群組的功能和設定許可權。 

1. 為 Viva Learning 啟用公開預覽功能 (預覽) 使用者。

    a. 修改 Teams 更新原則，以啟用公用預覽功能。 請參閱[Microsoft Teams 公開預覽](/microsoftteams/public-preview-doc-updates)。

    b. 針對將執行 Viva Learning (預覽) 測試的使用者或群組啟用更新原則。 請參閱 [將原則指派給使用者和群組](/microsoftteams/assign-policies-users-and-groups)。

2. 修改 Viva Learning (預覽) 使用者的應用程式許可權原則。
    ...
    b. 針對將執行 Viva Learning (預覽) 測試的使用者或群組，啟用 app 許可權原則。 請參閱 [將原則指派給使用者和群組](/microsoftteams/assign-policies-users-and-groups)。

3. 通知使用者將測試 Viva Learning (預覽) 將[其組建用戶端切換為 Teams 的公開預覽](set-up-teams-admin-center.md#user-steps-for-public-preview-tenants)。

> [!IMPORTANT]
> 針對公開預覽承租人，Viva Learning (Preview) 將不會顯示在 Teams 系統管理中心的 **受管理應用程式** 中，直到最後的產品發行為止。 不過，啟用 Public Preview 使用者可在設定正確的原則和許可權之後，于 Teams 應用程式存放區中找到 Viva Learning (Preview) ，並使用它。
...
3. 使用者現在可以在 Teams 應用程式存放區中找到 Viva Learning (預覽) ，然後開始使用它。
...
針對私人預覽承租人，不需要啟用其他原則。 不過，Viva Learning (Preview) 必須供組織中的使用者使用。
...
2. 在 [**管理應用程式**] 頁面上的搜尋方塊中，輸入 *Viva Learning*]，然後選取 [ **Viva Learning (預覽)**]。

   ![[管理應用程式] 頁面中顯示搜尋方塊的 Teams 系統管理中心。](../media/learning/learning-app-teams-manage-apps-page.png)

3. 在 [ **Viva Learning (預覽)** ] 頁面的 [**狀態**] 下，選取 [**允許**]，以開啟 Viva Learning (預覽) 。
...
[在 Microsoft 365 系統管理中心中設定 Viva Learning (Preview) 的教學內容來源](content-sources-365-admin-center.md)
