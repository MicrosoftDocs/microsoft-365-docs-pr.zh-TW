...
Microsoft 365 系統管理中心的管理員（自行或指派知識系統管理員角色給組織中所選的個人）可以管理與 Viva 學習 (Preview) 相關的設定，並且可以設定教學內容來源。
